...
Proposals are written to `KILN/.crucible/proposals/`. The `.crucible/` directory is excluded from indexing and file-watching, so a staged proposal never reaches [Precognition](./precognition/) or semantic search until a human accepts it. Writing `status: proposed` *into* the indexed kiln would surface unreviewed notes in retrieval — staging outside the index sidesteps that entirely.
...
- [Precognition](./precognition/) — the retrieval side of the knowledge loop
- [Agent Skills](./agent-skills/) — reflection can draft skill notes as proposals
- Product — where this fits in the product
