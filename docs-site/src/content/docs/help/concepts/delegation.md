...
2. Runs the task through the same scheduler every session uses — working tools, [Precognition](./precognition/) knowledge injection, Lua hooks, and standard persistence included
...
1. **[Agent cards](../extending/agent-cards/)** — specialized internal agents defined as markdown cards in your kiln, project, or config directory. This is the primary way to define delegation targets: a card carries its own system prompt, optional model, and per-tool policy.
2. **[Agent Client Protocol](./agent-client-protocol/) profiles** — external agents (Claude Code, OpenCode, Cursor, Gemini, Codex, or custom profiles from `crucible.toml`).
...
For more on how Crucible scopes agent permissions, see [Agent Client Protocol](./agent-client-protocol/).
...
- [Agent Cards](../extending/agent-cards/) for defining specialized delegation targets
- [Agent Client Protocol](./agent-client-protocol/) for the external-agent protocol
- [Agents & Protocols](./agents-and-protocols/) for an overview of agent architecture
- [Agent Skills](./agent-skills/) for how agents discover and load context
