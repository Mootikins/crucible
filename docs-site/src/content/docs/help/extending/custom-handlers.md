...
This guide explains how to create custom event handlers for the Crucible event system. For the simpler hook-based approach, see [Event Hooks](./event-hooks/).
...
> time. See [Event Hooks](./event-hooks/) for the events that actually fire.
...
Field names differ per event — see [Event Hooks](./event-hooks/) for each
event's payload.
