...
the existing [cru.sessions](./core/sessions/) primitives. This page
...
See [Sessions](./core/sessions/) for full signatures and the
[Lua Runtime API](./plugins/lua-runtime-api/) reference for return
shapes.
...
`cru.sessions.create`). See [Delegation](./concepts/delegation/) for how to
