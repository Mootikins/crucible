        ...
        if opener.open && opener.marker == closer.marker && !is_odd_match(&opener, &closer) {
            while closer.remaining > 0 && opener.remaining > 0 {
                ...
                if let Some(map) = opener_token.srcmap {
                    let (start, end) = map.get_byte_offsets();
                    opener_token.srcmap = Some(SourcePos::new(start, end.saturating_sub(marker_len)));
                    start_map_pos = end.saturating_sub(marker_len);
                }
...
#[cfg(test)]
mod tests {
    /// Regression test for upstream issue #48
    /// https://github.com/rlidwka/markdown-it.rs/issues/48
    ///
    /// Emphasis spanning lines in list items caused "attempt to subtract with overflow"
    /// panic in the original code.
    #[test]
    fn emphasis_across_list_lines_does_not_panic() {
