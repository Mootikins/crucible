...
async fn test_repl_system_tool_execution() -> Result<()> {
    ...
    let mut repl = timeout(
        Duration::from_secs(30),
        Repl::new(&config, None, None, "table".to_string()),
    )
    .await
...
async fn test_system_info_tool(repl: &Repl) -> Result<()> {
    ...
    assert!(
        tool_list.contains(&"system_info".to_string()),
...
async fn test_kiln_stats_tool(repl: &Repl, kiln: &TestKiln) -> Result<()> {
    ...
    if tool_list.contains(&"get_kiln_stats".to_string()) {
        ...
        let has_size_info =
            output.to_lowercase().contains("size") || output.to_lowercase().contains("bytes");

        assert!(
            has_notes_count || has_size_info,
...
async fn test_tools_listing(repl: &Repl) -> Result<()> {
    ...
    assert!(
        has_system_tools || has_rune_tools,
    ...
    for (group_name, tool_list) in &grouped_tools {
        assert!(
            !tool_list.is_empty(),
            "Tool group '{}' should not be empty",
            group_name
        );
...
async fn test_invalid_tool_execution(repl: &Repl) -> Result<()> {
    ...
    if let Some(first_tool) = tool_list.first() {
        ...
        let result = tools
            .execute_tool(first_tool, &["invalid_arg".to_string()])
            .await;

        // This might succeed or fail depending on the tool, but should not panic
        match result {
            Ok(_) => println!(
                "ℹ Tool '{}' accepted invalid arguments (may be expected)",
                first_tool
            ),
            Err(e) => println!(
...
async fn test_tool_execution_sequence(repl: &Repl) -> Result<()> {
                ...
                match tool_result.status {
                    crucible_cli::commands::repl::tools::ToolStatus::Success => {
                        ...
                        assert!(
                            ...
                            "Successful tool {} should produce output",
                            tool_name
                        );
                    }
    ...
    assert!(
        successful_executions > 0 || tool_list.is_empty(),
    ...
    println!(
        ...
        successful_executions, total_tools_to_test
    );
...
async fn test_repl_performance_stability() -> Result<()> {
    ...
    if !tool_list.is_empty() {
        ...
        assert!(
            ...
            "Multiple tool executions should complete within 30 seconds, took {:?}",
            execution_time
        );
...
async fn test_repl_configuration() -> Result<()> {
    ...
    let configs = vec![("table", None), ("json", None), ("csv", None)];
    ...
    for (format, db_path) in configs {
        ...
        let mut repl = timeout(
            Duration::from_secs(30),
            Repl::new(&config, db_path, None, format.to_string()),
        )
        .await
        .map_err(|_| anyhow::anyhow!("REPL creation timed out"))??;
        ...
        assert!(
            !repl.get_tools().list_tools().await.is_empty() || true, // Allow empty tool list
            "REPL should initialize correctly with {} format",
            format
        );
