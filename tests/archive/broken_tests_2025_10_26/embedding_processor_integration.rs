...
use crucible_surrealdb::{embedding_config::EmbeddingConfig, embedding_pool::EmbeddingThreadPool};
use crucible_watch::{
    create_embedding_metadata, EmbeddingEvent, EventDrivenEmbeddingConfig,
    EventDrivenEmbeddingProcessor, FileEventKind,
};
...
async fn test_basic_event_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let processor =
        EventDrivenEmbeddingProcessor::new(event_config, embedding_pool.clone()).await?;
    ...
    let metadata = create_embedding_metadata(
        &test_file,
...
async fn test_batch_processing() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        metrics.total_events_received, 4,
        "Should have received 4 events"
    );
    assert_eq!(
        metrics.total_events_processed, 4,
...
async fn test_event_deduplication() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(
        metrics.total_events_received, 3,
        "Should have received 3 events"
    );
    assert_eq!(
        metrics.deduplicated_events, 2,
        "Should have deduplicated 2 events"
    );
    assert_eq!(
        metrics.total_events_processed, 1,
        "Should have processed only 1 unique event"
    );
    ...
    assert_eq!(
        final_metrics.total_events_received, 4,
        "Should have received 4 events total"
    );
    assert_eq!(
        final_metrics.deduplicated_events, 2,
        "Should still have 2 deduplicated events"
    );
    assert_eq!(
        final_metrics.total_events_processed, 2,
        "Should have processed 2 unique events"
    );
...
async fn test_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        metrics.total_events_received >= 2,
        "Should have received at least 2 events"
    );
    // Some events might fail due to timeout, which is expected for this test
    info!(
        "Processed {} out of {} received events",
        metrics.total_events_processed, metrics.total_events_received
    );

    // Verify processor is still running and not shutdown due to errors
    assert!(
        !processor_arc.is_shutdown().await,
        "Processor should still be running after errors"
    );
