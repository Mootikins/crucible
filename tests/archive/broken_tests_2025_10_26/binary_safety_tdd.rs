...
impl BinarySafetyTestHarness {
    ...
    pub fn create_binary_file(&self, relative_path: &str, content: &[u8]) -> Result<String> {
        let full_path = self.test_kiln.kiln_path.join(relative_path);
    ...
    fn test_multiple_large_files_memory_safety() -> Result<()> {
        ...
        for result in results {
            ...
            assert!(
                ...
                "Content should be within memory limits"
            );
        }
    ...
    fn test_memory_usage_large_search() -> Result<()> {
        ...
        for i in 0..20 {
            let content = format!("# Document {}\n", i)
                + &"Large content block with searchable terms. ".repeat(5000);
            harness.create_text_file(&format!("doc_{}.md", i), &content)?;
        }

        // Search should complete without memory issues
        let results = search_files_in_kiln(harness.kiln_path(), "searchable", 50, false)?;
        ...
        for result in results {
            assert!(
                result.content.len() <= 1024 * 1024,
                "Each result content should be under 1MB"
            );
        }
