...
COLORTERM=truecolor COLORFGBG="15;0" tmux send-keys -t "$SESSION" \
  "asciinema rec '$CAST_FILE' --overwrite -c 'OPENAI_API_KEY=dummy ./crucible/target/debug/cru chat --replay $FIXTURE --replay-speed $SPEED --replay-auto-exit 500 -C assets/demo-config.toml'; exit" Enter
