...
## Purpose
This specification defines the simple HashMap-based tool registry implemented in Crucible, following production-validated patterns from successful agentic frameworks (LangChain, OpenAI Swarm, Anthropic, CrewAI). Implemented on 2025-11-04, the system eliminates unnecessary complexity by providing direct tool execution without caching, lifecycle management, or configuration services. The registry stores tools as simple async functions in a HashMap with O(1) lookup performance, parameter schema validation, and direct execution paths matching proven production patterns.
## Requirements
