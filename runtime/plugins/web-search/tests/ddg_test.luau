...
describe("ddg provider", function()
    describe("parse", function()
        ...
        it("drops sponsored rows, link and snippet alike", function()
            for _, r in ipairs(rows) do
                expect.falsy(r.url:find("example-ads.test", 1, true))
                expect.falsy((r.snippet or ""):find("Start free", 1, true))
            end
        end)

        it("unwraps the /l/?uddg= tracking hop to the real target", function()
            expect.equal("https://www.anthropic.com/news/prompt-caching", rows[2].url)
        end)

        it("pairs each snippet with the hit above it", function()
            expect.truthy(rows[1].snippet:find("Prompt caching lets you reuse", 1, true))
            expect.truthy(rows[4].snippet:find("Is there a crate", 1, true))
        end)

        it("strips markup out of snippets", function()
            expect.truthy(rows[1].snippet:find("CacheControl::Ephemeral", 1, true))
            expect.falsy(rows[1].snippet:find("<b>", 1, true))
        end)
        ...
        it("decodes entities in urls, which would otherwise be unfetchable", function()
            ...
            expect.equal("https://x.test/s?a=1&b=2", parsed[1].url)
        end)
        ...
        it("absolutises a protocol-relative href", function()
            ...
            expect.equal("https://x.test/page", parsed[1].url)
        end)
        ...
        it("does not depend on the order of the anchor's attributes", function()
            ...
            expect.equal("https://x.test/", parsed[1].url)
        end)
        ...
        it("says it was not served the lite page when the form is missing", function()
            ...
            expect.is_nil(parsed)
            expect.truthy(why:find("not the DuckDuckGo lite page", 1, true))
            expect.truthy(why:find("blocked, challenged or redirected", 1, true))
        end)
        ...
        it("says so when rows are present but no anchors matched", function()
            ...
            expect.deep_equal({}, parsed)
            expect.is_nil(why)
            expect.equal("no-anchors", note)
        end)

        it("reports a page with no rows at all as a markup change", function()
            local parsed, why = ddg.parse(lite_page(""))
            expect.is_nil(parsed)
            expect.truthy(why:find("has almost certainly changed", 1, true))
        end)
        ...
        it("parses a body packed with maximum-size rows in bounded time", function()
            ...
            expect.truthy(body:find("<tr", 1, true), "the body must reach the row loop")

            local started = os.clock()
            local rows = ddg.parse(body)
            local elapsed = os.clock() - started

            expect.truthy(rows, "a page of well-formed rows must parse")
        ...
        it("refuses a body over the parse limit", function()
            ...
            expect.is_nil(rows)
            expect.truthy(why:find("parse limit", 1, true), why)
        end)
        ...
        it("does not give a skipped row's snippet to the previous result", function()
            ...
            expect.equal(1, #rows, "the oversize row is dropped")
            expect.equal("https://a.test/", rows[1].url)
            expect.falsy(
        ...
        it("returns no rows, not a failure, when the page parsed but matched nothing", function()
            ...
            expect.is_nil(why)
            expect.deep_equal({}, parsed)
            expect.truthy(note, "the empty answer must carry why it is empty")
        end)

        it("rejects an empty body", function()
            local parsed, why = ddg.parse("")
            expect.is_nil(parsed)
            expect.truthy(why:find("empty response body", 1, true))
        end)
    end)
    ...
    describe("search", function()
        ...
        it("returns a payload that satisfies the contract", function()
            ...
            expect.equal("ddg", payload.provider)
        ...
        it("carries neither score nor engines, which are SearXNG-only", function()
            ...
            for _, r in ipairs(payload.results) do
                expect.is_nil(r.score)
                expect.is_nil(r.engines)
            end
        end)
        ...
        it("posts the query form-encoded to the lite endpoint", function()
            ...
            expect.equal(ddg.ENDPOINT, call[1])
            expect.equal("q=rust+prompt+caching", call[2].body)
            expect.equal("application/x-www-form-urlencoded", call[2].headers["Content-Type"])
        end)

        it("percent-encodes a query that would otherwise break the form body", function()
            serve(LITE_PAGE)
            assert(ddg.search("rust & lua: 100%"))
            expect.equal("q=rust+%26+lua%3A+100%25", last_post()[2].body)
        end)
        ...
        it("identifies itself honestly rather than spoofing a browser", function()
            ...
            expect.equal(ddg.USER_AGENT, ua)
            expect.truthy(ua:find("crucible", 1, true))
            expect.falsy(ua:find("Mozilla", 1, true))
        end)
        ...
        it("sends no authorization or cookie header, having no credential", function()
            ...
            for name in pairs(last_post()[2].headers) do
                ...
                expect.falsy(lower == "authorization")
                expect.falsy(lower == "cookie")
                expect.falsy(lower:find("key", 1, true))
            end
        end)
        ...
        it("honours a caller's timeout and otherwise applies its own", function()
            ...
            expect.equal(3, last_post()[2].timeout)
            assert(ddg.search("q"))
            expect.equal(ddg.DEFAULT_TIMEOUT, last_post()[2].timeout)
        end)

        it("passes max_results through to the contract", function()
            serve(LITE_PAGE)
            local payload = assert(ddg.search("q", { max_results = 2 }))
            expect.equal(2, #payload.results)
        end)
        ...
        it("fails with a structured error naming the provider, never a bare nil", function()
            ...
            expect.is_nil(payload)
            expect.is_table(err)
            expect.equal("ddg", err.provider)
            expect.truthy(err.message:find("ddg:", 1, true))
            expect.truthy(err.message:find("429", 1, true))
        end)

        it("names the endpoint it could not reach", function()
            serve("", { ok = false, status = 503 })
            local _, err = ddg.search("q")
            expect.truthy(err.message:find(ddg.ENDPOINT, 1, true))
        end)
        ...
        it("surfaces a parse failure as a provider failure", function()
            ...
            expect.is_nil(payload)
            expect.equal("ddg", err.provider)
            expect.truthy(err.message:find("not the DuckDuckGo lite page", 1, true))
        end)
        ...
        it("never quotes the response body back into the error", function()
            ...
            expect.falsy(reaching_the_model:find("IGNORE PREVIOUS", 1, true))
        end)
        ...
        it("fails when the page carries no rows at all", function()
            ...
            local payload, err = ddg.search("q")
            expect.is_nil(payload)
            expect.truthy(err.message:find("has almost certainly changed", 1, true))
        end)
        ...
        it("returns an empty result set, marked degraded, when nothing matched", function()
            ...
            expect.is_nil(err)
            expect.equal("ddg", payload.provider)
            expect.deep_equal({}, payload.results)
        ...
        it("fails when anchors parse but carry no title", function()
            ...
            local payload, err = ddg.search("q")
            expect.is_nil(payload)
            expect.truthy(err.message:find("title and a url", 1, true))
        end)
        ...
        it("rejects an empty query without making a request", function()
            ...
            expect.is_nil(payload)
            expect.equal("ddg", err.provider)
            expect.truthy(err.message:find("non-empty string", 1, true))
            expect.equal(before, #test_mocks.get_calls("http", "post"))
        end)
