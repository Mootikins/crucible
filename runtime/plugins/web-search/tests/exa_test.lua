...
describe("exa provider", function()
    describe("the recorded response", function()
        it("normalises to a valid contract payload", function()
            ...
            expect.equal("exa", payload.provider)
            expect.equal(3, #payload.results)
        ...
        it("truncates a long snippet to the contract's cap", function()
            ...
            expect.truthy(#payload.results[2].snippet <= contract.MAX_SNIPPET)
            expect.equal("...", payload.results[2].snippet:sub(-3))
        end)
    end)
    ...
    describe("the request", function()
        it("posts a tools/call envelope naming Exa's search tool", function()
            ...
            local url = last_post()
            expect.equal(URL, url)
            local sent = sent_envelope()
            expect.equal("2.0", sent.jsonrpc)
            expect.equal("tools/call", sent.method)
            expect.equal("web_search_exa", sent.params.name)
            expect.equal("rust prompt caching", sent.params.arguments.query)
        end)

        it("asks for the number of results the caller wanted", function()
            ok_body(recorded())
            exa("q", { max_results = 3 })
            expect.equal(3, sent_envelope().params.arguments.numResults)
        end)

        it("clamps numResults to the contract ceiling", function()
            ok_body(recorded())
            exa("q", { max_results = 999 })
            expect.equal(contract.MAX_RESULTS, sent_envelope().params.arguments.numResults)
        end)
        ...
        it("identifies itself honestly and takes an override", function()
            ...
            exa("q")
            local _, opts = last_post()
            expect.truthy(opts.headers["User-Agent"]:find("^crucible/"))

            exa("q", { user_agent = "crucible/9.9.9" })
            local _, with_override = last_post()
            expect.equal("crucible/9.9.9", with_override.headers["User-Agent"])
        end)
        ...
        it("sends no Authorization header when no key is configured", function()
            ...
            exa("q")
            local _, opts = last_post()
            expect.is_nil(opts.headers["Authorization"])
        end)
        ...
        it("sends the key as a header, never in the URL or body", function()
            ...
            expect.equal("Bearer exa-test-key-abcdef", opts.headers["Authorization"])
            ...
            expect.falsy(url:find("exa-test-key", 1, true))
            expect.falsy(opts.body:find("exa-test-key", 1, true))
        end)
        ...
        it("honours a timeout and defaults to one", function()
            ...
            exa("q", { timeout = 5 })
            local _, opts = last_post()
            expect.equal(5, opts.timeout)

            exa("q")
            local _, defaulted = last_post()
            expect.is_number(defaulted.timeout)
            expect.truthy(defaulted.timeout > 0)
        end)
    end)
    ...
    describe("failures name the provider and why", function()
        it("surfaces an HTTP status", function()
            ...
            expect.equal("exa", err.provider)
            expect.truthy(err.error:find("429", 1, true))
            expect.truthy(err.error:find("exa:", 1, true))
        end)
        ...
        it("surfaces a request that never reached the endpoint", function()
            ...
            expect.is_nil(payload)
            expect.equal("exa", err.provider)
            expect.truthy(err.error:find("failed", 1, true))
        end)
        ...
        it("surfaces a body that is not JSON-RPC", function()
            ...
            local payload, err = exa("q")
            expect.is_nil(payload)
            expect.truthy(err.error:find("not JSON%-RPC"))
        end)
        ...
        it("surfaces a JSON-RPC error object", function()
            ...
            local payload, err = exa("q")
            expect.is_nil(payload)
            expect.truthy(err.error:find("-32601", 1, true))
            expect.truthy(err.error:find("Method not found", 1, true))
        end)
        ...
        it("surfaces a tool-level error rather than reading it as results", function()
            ...
            local payload, err = exa("q")
            expect.is_nil(payload)
            expect.truthy(err.error:find("query is required", 1, true))
        end)
        ...
        it("surfaces a result block that is not an Exa search payload", function()
            ...
            local payload, err = exa("q")
            expect.is_nil(payload)
            expect.truthy(err.error:find("Exa search payload", 1, true))
        end)
        ...
        it("rejects an empty query without calling out", function()
            ...
            expect.is_nil(payload)
            expect.equal("exa", err.provider)
            expect.truthy(err.error:find("query", 1, true))
        ...
        it("scrubs a key the endpoint echoed back", function()
            ...
            expect.falsy(err.error:find(key, 1, true))
            expect.truthy(err.error:find("redacted", 1, true))
        end)
    end)
    ...
    describe("transport variations", function()
        ...
        it("treats no matches as an empty result set, not a failure", function()
            ...
            expect.truthy(contract.validate(payload))
        end)
        ...
        it("drops hits with no url, keeping the rest", function()
            ...
            expect.equal("kept", payload.results[1].title)
        end)
    end)
end)
