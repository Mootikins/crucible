...
describe("charge", function()
    it("allows turns up to the cap", function()
        with_config(CAP_1, function()
            expect.equals(true, quota.charge("under", "2026-01-01"))
        end)
    end)
    ...
    it("refuses every turn past the cap and replies exactly once", function()
        with_config(CAP_1, function()
            ...
            expect.deep_equal({ true, false, false }, allowed)
            expect.equals(1, replies)
        end)
    end)
    ...
    it("names the cap in the one reply it sends", function()
        with_config(CAP_1, function()
            ...
            expect.is_not_nil(refusal:find("1", 1, true))
        end)
    end)
    ...
    it("counts each user separately", function()
        with_config(CAP_1, function()
            ...
            expect.equals(false, quota.charge("noisy", "2026-01-01"))
            expect.equals(true, quota.charge("quiet", "2026-01-01"))
        end)
    end)
    ...
    it("resets on the next day, reply included", function()
        with_config(CAP_1, function()
            ...
            expect.equals(false, quota.charge("daily", "2026-01-01"))

            expect.equals(true, quota.charge("daily", "2026-01-02"))
            local ok, refusal = quota.charge("daily", "2026-01-02")
            expect.equals(false, ok)
            expect.is_not_nil(refusal)
        end)
    end)
    ...
    it("shares one bucket for messages with no author id", function()
        with_config(CAP_1, function()
            expect.equals(true, quota.charge(nil, "2026-01-01"))
            expect.equals(false, quota.charge(nil, "2026-01-01"))
        end)
    end)
    ...
    it("falls back to the plugin.yaml default when the key is unset", function()
        with_config({}, function()
            for _ = 1, 50 do
                expect.equals(true, quota.charge("defaulted", "2026-01-01"))
            end
            expect.equals(false, quota.charge("defaulted", "2026-01-01"))
        end)
    end)
end)
