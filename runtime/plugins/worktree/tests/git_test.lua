...
describe("validate_branch", function()
  it("accepts ordinary branch names, slashes and all", function()
    expect.truthy(git.validate_branch("main"))
    expect.truthy(git.validate_branch("feat/container-isolation"))
    expect.truthy(git.validate_branch("release-0.20.1"))
  end)
  ...
  it("refuses path traversal and separators", function()
    expect.falsy(git.validate_branch("../../etc/passwd"))
    expect.falsy(git.validate_branch("/absolute"))
    expect.falsy(git.validate_branch("back\\slash"))
  end)

  it("refuses an empty or absent name", function()
    expect.falsy(git.validate_branch(""))
    expect.falsy(git.validate_branch(nil))
  end)
end)

describe("worktree_dest", function()
  it("substitutes both placeholders", function()
    expect.equals("/repo/tree/feat/x", git.worktree_dest(nil, "/repo", "feat/x"))
  end)
  ...
  it("survives a percent sign in the repo path", function()
    expect.equals("/tmp/100%/tree/main", git.worktree_dest(nil, "/tmp/100%", "main"))
  end)
end)
...
describe("parse_worktrees", function()
  ...
  it("maps each branch to its checkout", function()
    ...
    expect.equals("/repo", map["master"])
    expect.equals("/repo/tree/feat/x", map["feat/x"])
  end)
  ...
  it("reports the main checkout as the first record", function()
    expect.equals("/repo", git.main_worktree(porcelain))
  end)

  it("survives empty input", function()
    expect.equals(0, #git.lines(""))
    local map = git.parse_worktrees("")
    expect.equals(nil, next(map))
  end)
end)

describe("strip_remote", function()
  it("drops the remote segment", function()
    expect.equals("feat/x", git.strip_remote("origin/feat/x"))
    expect.equals("main", git.strip_remote("upstream/main"))
  end)
  ...
  it("refuses the symbolic HEAD entry", function()
    expect.equals(nil, git.strip_remote("origin/HEAD"))
  end)

  it("refuses a name with no remote segment", function()
    expect.equals(nil, git.strip_remote("main"))
  end)
end)
...
describe("build_branches", function()
  ...
  it("marks the current branch, and only it", function()
    ...
    expect.equals(1, #current)
    expect.equals("feat/x", current[1])
  end)
  ...
  it("attaches an existing checkout to the branch that owns it", function()
    ...
    for _, b in ipairs(branches) do
      if b.name == "feat/x" then expect.equals("/repo/tree/feat/x", b.worktree_path) end
      if b.name == "zebra" then expect.equals(nil, b.worktree_path) end
    end
  end)
  ...
  it("includes a branch that exists only on a remote", function()
    ...
    expect.truthy(found)
    expect.equals(true, found.remote_only)
  end)
  ...
  it("orders current, then checked-out, then the rest", function()
    ...
    expect.equals("master", names[1])
    expect.equals("feat/x", names[2])
    expect.equals("only-remote", names[3])
    expect.equals("zebra", names[4])
  end)
  ...
  it("reports no current branch on a detached HEAD", function()
    ...
    expect.equals(nil, current)
    for _, b in ipairs(branches) do expect.falsy(b.is_current) end
  end)
end)
...
describe("to_targets", function()
  ...
  it("says what picking each row will do", function()
    ...
    expect.equals("new worktree", targets[4].hint)
  end)
