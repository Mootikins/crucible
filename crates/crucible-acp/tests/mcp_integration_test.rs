...
async fn test_mcp_server_with_env_variables() {
    ...
    let env_vars = vec![
        ...
        EnvVariable {
            name: "KILN_PATH".to_string(),
            value: "/path/to/kiln".to_string(),
            meta: None,
        },
    ];
    ...
    match &mcp_server {
        McpServer::Stdio { name: _, command: _, args: _, env } => {
            ...
            assert_eq!(env[1].name, "KILN_PATH");
            assert_eq!(env[1].value, "/path/to/kiln");
        }
...
async fn test_multiple_mcp_servers() {
    ...
    let request = NewSessionRequest {
        cwd: PathBuf::from("/test"),
        mcp_servers: vec![crucible_server, another_server],
        meta: None,
    };
