...
/// Test database backend module
///
/// This module provides helpers for using real SurrealDB in tests instead of mocks.
/// Use real SurrealDB for integration tests that need full query support (graph traversal, etc.)
pub mod real_db {
    use surrealdb::engine::local::{Db, Mem};
    use surrealdb::Surreal;

    /// Create a real in-memory SurrealDB instance for integration testing
    ///
    /// This uses the actual SurrealDB Rust SDK with in-memory backend,
    /// providing full query support including graph traversal, RELATE statements, etc.
    pub async fn setup_real_test_db() -> anyhow::Result<Surreal<Db>> {
...
impl EmbeddingTestUtils {
    ...
    /// Creates 1 main embedding + embeddings_per_doc chunk embeddings for each document
    pub fn create_embedding_batch(
        ...
        for &document_id in document_ids.iter() {
            // Create main embedding (no chunk_id)
            let main_embedding = Self::create_document_embedding(document_id, dimensions);
            embeddings.push(main_embedding);
            ...
            for chunk_index in 0..embeddings_per_doc {
                let embedding = Self::create_chunk_embedding(
                    ...
                    chunk_index, // Use chunk_index as position for chunks
                    dimensions,
                );
