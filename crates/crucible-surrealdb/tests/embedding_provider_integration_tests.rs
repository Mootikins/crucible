...
use crucible_surrealdb::embedding_pool::{EmbeddingThreadPool, create_embedding_thread_pool};
use crucible_surrealdb::embedding_config::{EmbeddingConfig, EmbeddingModel, PrivacyMode};
use crucible_llm::embeddings::{EmbeddingProvider, EmbeddingConfig as LlmEmbeddingConfig, ProviderType};
...
async fn test_ollama_provider_real_embedding_generation() {
    ...
    let result = pool.process_document_with_retry("test_doc_ollama", test_content).await.unwrap();

    // DEBUG: Print result information
    println!("Result: succeeded={}, attempt_count={}, total_time={:?}",
    ...
    assert!(result.succeeded, "Document processing should succeed with Ollama");
    assert!(result.attempt_count >= 1, "Should attempt at least once");
    assert!(result.total_time.as_millis() > 0, "Processing should take time");
...
async fn test_openai_provider_real_embedding_generation() {
    ...
    let result = pool.process_document_with_retry("test_doc_openai", test_content).await.unwrap();

    // ASSERT: Verify we get REAL embeddings from OpenAI
    assert!(result.succeeded, "Document processing should succeed with OpenAI");
    assert!(result.attempt_count >= 1, "Should attempt at least once");
    assert!(result.total_time.as_millis() > 0, "Processing should take time");
...
async fn test_provider_switching_based_on_configuration() {
    ...
    let original_provider = std::env::var("EMBEDDING_PROVIDER").unwrap_or_else(|_| "ollama".to_string());
    ...
    let pool_config = create_test_pool_config();
    let ollama_pool = create_embedding_thread_pool(pool_config.clone()).await.unwrap();

    let test_content = "Test content for provider switching";
    let ollama_result = ollama_pool.process_document_with_retry("test_switch", test_content).await.unwrap();
    ...
    if std::env::var("OPENAI_API_KEY").is_ok() {
        ...
        let openai_result = openai_pool.process_document_with_retry("test_switch", test_content).await.unwrap();
...
async fn test_batch_processing_with_real_embeddings() {
    ...
    let documents = vec![
        ("doc1".to_string(), "First test document about machine learning and AI".to_string()),
        ("doc2".to_string(), "Second test document about Rust programming and systems".to_string()),
        ("doc3".to_string(), "Third test document about knowledge management and collaboration".to_string()),
        ("doc4".to_string(), "Fourth test document about vector embeddings and similarity search".to_string()),
        ("doc5".to_string(), "Fifth test document about CRDTs and real-time synchronization".to_string()),
    ];
    ...
    assert_eq!(result.processed_count, 5, "All documents should be processed");
    assert_eq!(result.failed_count, 0, "No documents should fail");
    assert_eq!(result.embeddings_generated, 5, "Should generate 5 embeddings");
    assert!(!result.circuit_breaker_triggered, "Circuit breaker should not trigger");
    assert!(result.total_processing_time.as_millis() > 0, "Processing should take time");
    assert!(result.total_processing_time.as_millis() > 100, "Real embedding generation takes time");
...
async fn test_error_handling_for_embedding_service_failures() {
    ...
    std::env::set_var("EMBEDDING_ENDPOINT", "http://invalid-endpoint-that-does-not-exist.local:12345");
    std::env::set_var("EMBEDDING_PROVIDER", "ollama");
    std::env::set_var("EMBEDDING_MODEL", "nomic-embed-text");
    ...
    let pool = create_embedding_thread_pool(pool_config.clone()).await.unwrap();

    let test_content = "This should fail with invalid endpoint";

    // ACT: Attempt to process document with invalid endpoint
    let result = pool.process_document_with_retry("test_failure", test_content).await.unwrap();

    // ASSERT: Verify proper error handling
    assert!(!result.succeeded, "Document processing should fail with invalid endpoint");
    ...
    assert!(!error.error_message.is_empty(), "Error should have meaningful message");
    ...
    if metrics.failed_tasks >= pool_config.circuit_breaker_threshold as u64 {
        ...
        assert!(metrics.circuit_breaker_open, "Circuit breaker should be open after multiple failures");
    }

    pool.shutdown().await.unwrap();

    // Restore valid endpoint
    std::env::set_var("EMBEDDING_ENDPOINT", "https://llm.example.com");
}
...
async fn test_configuration_validation() {
    ...
    assert!(llm_config_result.is_err(), "Should fail with invalid provider type");
    ...
    let llm_config_result = LlmEmbeddingConfig::from_env();
    assert!(llm_config_result.is_err(), "Should fail without API key for OpenAI");
    ...
    let llm_config_result = LlmEmbeddingConfig::from_env();
    assert!(llm_config_result.is_ok(), "Should succeed with valid Ollama config");
...
async fn test_embedding_dimensions_match_provider_specifications() {
    ...
    for (provider, model, expected_dims) in test_cases {
        ...
        let result = pool.process_document_with_retry("test_dims", &test_content).await.unwrap();

        // ASSERT: Verify dimensions match specification
        assert!(result.succeeded, "Should succeed with {}/{}", provider, model);
...
async fn test_concurrent_embedding_generation() {
    ...
    let documents: Vec<(String, String)> = (0..10)
        .map(|i| (
            ...
            format!("This is concurrent test document number {} with unique content {}", i, i * 42)
        ))
        .collect();
    ...
    assert_eq!(result.processed_count, 10, "All documents should be processed");
    assert_eq!(result.failed_count, 0, "No documents should fail");
    assert_eq!(result.embeddings_generated, 10, "Should generate 10 embeddings");

    // Concurrent processing should be faster than sequential
    // This is a rough estimate - real embedding generation takes time
    assert!(elapsed.as_secs() < 120, "Concurrent processing should complete in reasonable time");
...
async fn test_mock_vs_real_embedding_differences() {
    ...
    let result1 = pool.process_document_with_retry("mock_test_1", test_content1).await.unwrap();
    let result2 = pool.process_document_with_retry("mock_test_2", test_content2).await.unwrap();
    ...
    assert!(result1.total_time.as_millis() > 10, "Real embedding generation should take time, not be instant");
    assert!(result2.total_time.as_millis() > 10, "Real embedding generation should take time, not be instant");

    // Also verify that retry logic works (indicating real network behavior)
    // Mock implementation would never need retries
    let metrics = pool.get_metrics().await;
    assert!(metrics.total_tasks_processed >= 2, "Should have processed both documents");
...
async fn test_provider_health_check_integration() {
    ...
    let result = pool.process_document_with_retry("health_check", "Health check test content").await.unwrap();
    ...
    if result.succeeded {
        ...
        assert!(result.final_error.is_some(), "Should have error details for health check failure");
        let error = result.final_error.unwrap();
        println!("Health check error: {}", error.error_message);

        // Verify error is service-related, not configuration-related
        assert!(!error.error_message.contains("configuration"),
