...
use crucible_surrealdb::{
    SurrealClient,
    vault_integration,
    embedding_config::DocumentEmbedding,
};
use crucible_llm::embeddings::{EmbeddingProvider, EmbeddingResponse, create_mock_provider};
use vault_integration::{
    store_document_embedding, get_document_embeddings, clear_document_embeddings,
    semantic_search, initialize_vault_schema, get_database_stats, get_all_document_embeddings
};
use chrono::Utc;
use std::sync::Arc;
...
fn calculate_cosine_similarity(vec_a: &[f32], vec_b: &[f32]) -> f64 {
    ...
    let dot_product: f64 = vec_a.iter().zip(vec_b.iter()).map(|(a, b)| *a as f64 * *b as f64).sum();
    let magnitude_a: f64 = vec_a.iter().map(|x| (*x as f64) * (*x as f64)).sum::<f64>().sqrt();
    let magnitude_b: f64 = vec_b.iter().map(|x| (*x as f64) * (*x as f64)).sum::<f64>().sqrt();
...
async fn test_cosine_similarity_calculation_basic() {
    ...
    initialize_vault_schema(&client).await.expect("Failed to initialize schema");
    ...
    assert!((similarity - 0.7071068).abs() < 1e-6, "45-degree vectors should have similarity ~0.707");
}
...
async fn test_cosine_similarity_different_dimensions() {
    ...
    assert!((similarity_256 - similarity_768).abs() < 0.01, "Similarities should be dimension-agnostic");
    assert!((similarity_768 - similarity_1536).abs() < 0.01, "Similarities should be dimension-agnostic");

    // All should be reasonably high similarity
    assert!(similarity_256 > 0.8, "High similarity expected: {}", similarity_256);
    assert!(similarity_768 > 0.8, "High similarity expected: {}", similarity_768);
    assert!(similarity_1536 > 0.8, "High similarity expected: {}", similarity_1536);
}
...
async fn test_cosine_similarity_edge_cases() {
    ...
    assert_eq!(similarity, 0.0, "Zero vector should have similarity 0.0 with any non-zero vector");

    // Test two zero vectors
    let similarity = calculate_cosine_similarity(&zero_vec, &zero_vec);
    assert_eq!(similarity, 0.0, "Two zero vectors should have similarity 0.0");
    ...
    assert!((similarity - 0.0).abs() < 1e-6, "Mixed vectors should have similarity 0.0");
}
...
async fn test_query_embedding_generation_mock_provider() {
    ...
    let embedding_response = mock_provider.embed(query).await.expect("Failed to generate query embedding");

    assert_eq!(embedding_response.dimensions, 768, "Query embedding should have 768 dimensions");
    assert!(!embedding_response.embedding.is_empty(), "Query embedding should not be empty");
    assert!(embedding_response.embedding.len() == 768, "Embedding vector should match dimensions");

    // Test consistency - same query should generate same embedding
    let embedding_response2 = mock_provider.embed(query).await.expect("Failed to generate query embedding");
    assert_eq!(embedding_response.embedding, embedding_response2.embedding, "Same query should generate same embedding");

    // Test different queries generate different embeddings
    let different_query = "natural language processing";
    let different_embedding = mock_provider.embed(different_query).await.expect("Failed to generate query embedding");
    assert_ne!(embedding_response.embedding, different_embedding.embedding, "Different queries should generate different embeddings");
}
...
async fn test_query_embedding_batch_generation() {
    ...
    let embedding_responses = mock_provider.embed_batch(queries.clone()).await.expect("Failed to generate batch embeddings");

    assert_eq!(embedding_responses.len(), 3, "Should generate embeddings for all queries");

    for (i, response) in embedding_responses.iter().enumerate() {
        assert_eq!(response.dimensions, 768, "All embeddings should have 768 dimensions");
        assert!(!response.embedding.is_empty(), "Embedding should not be empty");
        assert_eq!(response.embedding.len(), 768, "Embedding vector should match dimensions");

        // Verify consistency
        let single_response = mock_provider.embed(&queries[i]).await.expect("Failed to generate single embedding");
        assert_eq!(response.embedding, single_response.embedding, "Batch embedding should match single embedding");
    }
}
...
async fn test_semantic_search_basic_vector_similarity() {
    ...
    for embedding in &test_embeddings {
        store_document_embedding(&client, embedding).await.expect("Failed to store test embedding");
    }
    ...
    let search_results = semantic_search(&client, query, 5).await.expect("Semantic search should succeed");

    // Verify results structure
    assert!(!search_results.is_empty(), "Search should return some results");
    ...
    for (_, score) in &search_results {
        assert!(*score >= -1.0 && *score <= 1.0,
               "Similarity scores should be between -1.0 and 1.0, got: {}", score);
    }
...
async fn test_semantic_search_ranking_accuracy() {
    ...
    let search_results = semantic_search(&client, "machine learning", 10).await.expect("Semantic search should succeed");
    ...
    expected_similarities.sort_by(|a, b| b.1.partial_cmp(&a.1).unwrap_or(std::cmp::Ordering::Equal));
...
async fn test_semantic_search_different_embedding_dimensions() {
    ...
    let search_results = semantic_search(&client, "test query", 10).await.expect("Semantic search should succeed");

    // Should find results regardless of embedding dimensions
    assert!(!search_results.is_empty(), "Search should find results across different dimensions");

    // Each result should have a valid similarity score
    for (_, score) in &search_results {
        assert!(*score >= -1.0 && *score <= 1.0,
               "Similarity scores should be valid for all dimensions: {}", score);
    }
}
...
async fn test_semantic_search_similarity_threshold_filtering() {
    ...
    let search_results = semantic_search(&client, "machine learning", 3).await.expect("Semantic search should succeed");

    // Should respect the limit
    assert!(search_results.len() <= 3, "Search should respect result limit");
    ...
    if !search_results.is_empty() {
        ...
        assert!(search_results[0].1 > 0.3,
               "Top results should have reasonable similarity: {}", search_results[0].1);
    }
}
...
async fn test_batch_similarity_search_multiple_queries() {
    ...
    for (query, results) in &all_results {
        ...
        for i in 1..results.len() {
            assert!(results[i-1].1 >= results[i].1,
                   "Results for query '{}' should be sorted", query);
        }
    }
    ...
    for (_, results) in &all_results {
        let signature: Vec<String> = results.iter()
...
async fn test_semantic_search_empty_database() {
    ...
    let search_results = semantic_search(&client, "any query", 10).await.expect("Search should succeed even on empty database");

    assert!(search_results.is_empty(), "Empty database should return no results");
}
...
async fn test_semantic_search_empty_query() {
    ...
    let search_results = semantic_search(&client, "", 10).await.expect("Search should handle empty query gracefully");
    ...
    if !search_results.is_empty() {
        ...
        for (_, score) in &search_results {
            assert!(*score >= -1.0 && *score <= 1.0,
                   "Similarity scores should be valid even for empty queries: {}", score);
        }
    }
}
...
async fn test_semantic_search_missing_embeddings() {
    ...
    let create_doc_sql = "CREATE notes SET title = 'Test Doc', content = 'Test content', path = 'test.md'";
    client.query(create_doc_sql, &[]).await.expect("Failed to create test document");

    // Wait for document creation
    tokio::time::sleep(tokio::time::Duration::from_millis(50)).await;

    // Search for documents without embeddings
    let search_results = semantic_search(&client, "test", 10).await.expect("Search should handle missing embeddings gracefully");
    ...
    if !search_results.is_empty() {
        ...
        for (_, score) in &search_results {
            assert!(*score >= -1.0 && *score <= 1.0,
                   "Similarity scores should be valid even with missing embeddings: {}", score);
        }
    }
}
    ...
    match search_results {
        Ok(results) => {
            ...
            for (_, score) in &results {
                assert!(*score >= -1.0 && *score <= 1.0,
                       "Similarity scores should be valid: {}", score);
            }
        }
        Err(_) => {
...
async fn test_semantic_search_performance_large_dataset() {
    ...
    println!("Stored {} embeddings in {:?}", num_documents, store_duration);
    ...
    let search_results = semantic_search(&client, "performance test query", 10).await.expect("Semantic search should succeed");
    let search_duration = search_start.elapsed();

    println!("Semantic search on {} documents completed in {:?} with {} results",
             num_documents, search_duration, search_results.len());

    // Performance assertions
    assert!(search_duration.as_millis() < 5000, "Search should complete within 5 seconds, took {:?}", search_duration);
    ...
    for (_, score) in &search_results {
        assert!(*score >= -1.0 && *score <= 1.0, "Similarity scores should be valid: {}", score);
    }
}
...
async fn test_semantic_search_concurrent_queries() {
    ...
    for query in queries {
        ...
        let task = tokio::spawn(async move {
            semantic_search(&client_clone, query, 5).await
        });
    ...
    println!("Concurrent searches ({}) completed in {:?}", all_results.len(), concurrent_duration);

    // Each search should have returned valid results
    for results in &all_results {
        assert!(!results.is_empty(), "Each concurrent search should return results");

        // Results should be sorted by similarity
        for i in 1..results.len() {
            assert!(results[i-1].1 >= results[i].1, "Concurrent search results should be sorted");
        }
    }

    // Concurrent searches should be reasonably efficient
    assert!(concurrent_duration.as_millis() < 10000, "Concurrent searches should complete within 10 seconds");
}
...
async fn test_vector_search_integration_with_database_stats() {
    ...
    let initial_stats = get_database_stats(&client).await.expect("Should get initial stats");
    assert_eq!(initial_stats.total_documents, 0, "Should start with no documents");
    assert_eq!(initial_stats.total_embeddings, 0, "Should start with no embeddings");
    ...
    // Check updated stats
    let updated_stats = get_database_stats(&client).await.expect("Should get updated stats");
    assert!(updated_stats.total_embeddings > 0, "Should have stored embeddings");

    // Perform semantic search
    let search_results = semantic_search(&client, "test query", 5).await.expect("Semantic search should succeed");

    // Search should find results that match stored embeddings count
    assert!(!search_results.is_empty(), "Search should find stored embeddings");

    // Number of results should not exceed stored embeddings
    assert!(search_results.len() <= updated_stats.total_embeddings as usize,
...
async fn test_vector_search_integration_with_embedding_retrieval() {
    ...
    let stored_embeddings = get_document_embeddings(&client, "doc1").await.expect("Should retrieve stored embeddings");
    assert!(!stored_embeddings.is_empty(), "Should have stored embeddings for doc1");
    ...
    assert!(doc1_found, "Search should find doc1 which has stored embeddings");
    ...
    if let Some((_, similarity_score)) = search_results.iter().find(|(doc_id, _)| doc_id == "doc1") {
        assert!(*similarity_score >= -1.0 && *similarity_score <= 1.0,
               "Similarity score should be valid: {}", similarity_score);
    }
}
...
async fn test_current_implementation_is_mock_text_search() {
    ...
    let search_results = semantic_search(&client, "machine", 10).await.expect("Search should succeed");
    ...
    if !search_results.is_empty() {
        ...
        println!("Current mock implementation returned {} results", search_results.len());

        // The scores should be from mock similarity calculation
        for (_, score) in &search_results {
            assert!(*score >= 0.0 && *score <= 1.0,
                   "Mock implementation scores should be between 0.0 and 1.0: {}", score);
        }
    }
    ...
    println!("EXPECTED BEHAVIOR AFTER IMPLEMENTATION: semantic_search should use vector similarity");
}