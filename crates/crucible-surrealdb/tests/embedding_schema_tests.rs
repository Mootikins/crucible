...
use crucible_surrealdb::{SurrealEmbeddingDatabase, EmbeddingMetadata, EmbeddingData, Document, SearchQuery, SearchFilters};
use chrono::Utc;
...
fn create_test_metadata(file_path: &str, model: &str) -> EmbeddingMetadata {
    EmbeddingMetadata {
        ...
        tags: vec!["test".to_string(), "embedding".to_string(), model.to_string()],
...
async fn test_embedding_schema_exists_and_functions() {
    ...
    db.store_embedding(
        ...
        &embedding,
        &metadata,
    ).await.expect("Should store embedding with 384 dimensions");

    // Verify the embedding was stored correctly
    let stored_data = db.get_embedding("test_schema.md")
        .await
        .expect("Should retrieve stored embedding")
        .expect("Embedding should exist");

    assert_eq!(stored_data.embedding.len(), 384, "Embedding should have exactly 384 dimensions");
    assert_eq!(stored_data.embedding, embedding, "Stored embedding should match original");
    assert_eq!(stored_data.metadata.properties.get("model"), Some(&serde_json::json!("all-MiniLM-L6-v2")));
    ...
    let updated_data = db.get_embedding("test_schema.md")
        .await
        .expect("Should retrieve updated embedding")
        .expect("Embedding should still exist after metadata update");

    assert_eq!(updated_data.embedding, embedding, "Embedding should remain unchanged after metadata update");
    assert_eq!(updated_data.metadata.tags, vec!["test", "embedding", "text-embedding-ada-002"]);

    // Test 3: Verify file existence checking works
    println!("Testing file existence validation...");

    assert!(db.file_exists("test_schema.md").await.expect("Should check file existence"));
    assert!(!db.file_exists("nonexistent.md").await.expect("Should return false for nonexistent file"));

    // Test 4: Verify search functionality with embeddings
    println!("Testing basic search functionality...");

    let search_results = db.search_similar("test", &embedding, 5)
        .await
    ...
    assert!(search_results[0].score > 0.99, "Should have very high similarity for identical embedding");
...
async fn test_vector_similarity_search() {
    ...
    let documents = vec![
        ("doc1.md", "Machine learning fundamentals", vec![1.0f32; 384]),
        ("doc2.md", "Deep learning neural networks", vec![0.9f32; 384]),
        ("doc3.md", "Rust programming language", vec![0.5f32; 384]),
        ("doc4.md", "Database design patterns", vec![0.4f32; 384]),
        ("doc5.md", "Web development with JavaScript", vec![0.1f32; 384]),
    ];
    ...
    let results = db.search_similar("machine learning", &query_embedding, 5)
        .await
        .expect("Should perform similarity search");
    ...
    for i in 1..results.len() {
        assert!(results[i-1].score >= results[i].score, "Results should be sorted by decreasing similarity");
    }
    ...
    for result in &results {
        ...
        assert!(!result.content.is_empty(), "Result should have valid content");
    ...
    let limited_results = db.search_similar("test", &query_embedding, 2)
        .await
    ...
    let rust_results = db.search_similar("rust programming", &rust_embedding, 3)
        .await
        .expect("Should find programming documents");

    assert!(!rust_results.is_empty(), "Should find results for rust query");

    // Should find some documents in the search results
    let rust_ids: Vec<String> = rust_results.iter().map(|r| r.id.clone()).collect();
    assert!(!rust_ids.is_empty(), "Should find documents in search results");
    ...
    assert!(found_any, "Should find at least one of the original documents");
    ...
    empty_db.initialize().await.expect("Empty database initialization should succeed");

    let empty_results = empty_db.search_similar("test", &query_embedding, 5)
        .await
        .expect("Should handle empty database search");

    assert!(empty_results.is_empty(), "Empty database should return no results");
...
async fn test_embedding_workflow_integration() {
    ...
    let doc1 = create_test_document("workflow_test.md", "Initial content for workflow test", "model-v1");
    ...
    let search_results = db.search_similar("workflow", &doc1.embedding, 5)
        .await
    ...
    let batch_result = db.batch_operation(&batch_operation)
        .await
    ...
    let filtered_results = db.search(&search_query)
        .await
        .expect("Should perform filtered search");

    assert_eq!(filtered_results.len(), 3, "Should find all 3 batch documents");
    ...
    updated_metadata.properties.insert("version".to_string(), serde_json::json!(2));

    db.update_metadata("workflow_test.md", &updated_metadata)
        .await
        .expect("Should update metadata");

    let updated_data = db.get_embedding("workflow_test.md")
        .await
    ...
    assert_eq!(updated_data.metadata.properties.get("version"), Some(&serde_json::json!(2)));
    ...
    // Delete a file
    let deleted = db.delete_file("batch2.md").await.expect("Should delete file");
    assert!(deleted);
...
async fn test_embedding_performance_characteristics() {
    ...
    assert!(batch_duration.as_secs() < 5, "Large batch should complete within 5 seconds");
    ...
    let search_results = db.search_similar("performance test", &query_embedding, 10)
        .await
        .expect("Should perform search efficiently");

    let search_duration = search_start.elapsed();

    assert_eq!(search_results.len(), 10, "Should return requested number of results");
    println!("Similarity search took: {:?}", search_duration);
    assert!(search_duration.as_millis() < 1000, "Search should complete within 1 second");
    ...
    for i in 0..10 {
        ...
        let search_results = db.search_similar(&format!("sequential query {}", i), &query_emb, 5)
            .await
            .expect("Search should succeed");

        assert_eq!(search_results.len(), 5, "Should find 5 results for query {}", i);
    }

    let sequential_duration = sequential_start.elapsed();

    println!("10 sequential searches took: {:?}", sequential_duration);
    assert!(sequential_duration.as_secs() < 3, "Sequential searches should complete within 3 seconds");
    ...
    assert!(final_stats.storage_size_bytes.unwrap_or(0) > 0, "Storage size should be greater than zero");

    // Should include all the data we stored
    println!("Storage size: {} bytes for {} documents",
        final_stats.storage_size_bytes.unwrap_or(0), final_stats.total_documents);
...
async fn test_embedding_error_handling() {
    ...
    db.store_embedding("invalid_dim.md", "Invalid dimensions", &invalid_embedding, &metadata)
        .await
    ...
    assert!(update_result.is_err(), "Should fail to update nonexistent file");

    let nonexistent_data = db.get_embedding("nonexistent.md").await.expect("Should handle get operation");
    assert!(nonexistent_data.is_none(), "Should return None for nonexistent file");

    let delete_result = db.delete_file("nonexistent.md").await.expect("Should handle delete operation");
    ...
    db.store_embedding("empty.md", "Empty embedding", &empty_embedding, &empty_metadata)
        .await
        .expect("Should handle empty embedding");

    // Search with empty embedding should still work
    let empty_results = db.search_similar("empty test", &empty_embedding, 5)
        .await
    ...
    assert!(batch_result.successful >= 2, "Should process valid documents");
...
async fn test_embedding_index_functionality() {
    ...
    let test_cases = vec![
        ...
        ("dissimilar.md", "Dissimilar content", create_test_embedding(2000)),
        ("very_dissimilar.md", "Very dissimilar content", create_test_embedding(3000)),
    ];
    ...
    let search_results = db.search_similar("base query", &base_embedding, 10)
        .await
    ...
    let high_similarity_results: Vec<_> = search_results.iter()
        .filter(|r| r.score > 0.95)
        .collect();

    assert!(!high_similarity_results.is_empty(), "Should find at least one high-similarity result");
    ...
    for original_id in &original_ids {
        assert!(result_ids.contains(original_id), "Should find {} in search results", original_id);
    }

    // All results should have valid scores (allowing for small floating point precision issues)
    for result in &search_results {
        assert!(result.score >= 0.0 && result.score <= 1.1, "Score should be between 0 and 1.1 (allowing for precision)");
    }

    // Test 3: Test with different top_k values
    println!("Testing different result limits...");

    let top_2_results = db.search_similar("test", &base_embedding, 2)
        .await
    ...
    let original_ids = vec!["identical.md", "very_similar.md", "moderately_similar.md", "dissimilar.md", "very_dissimilar.md"];
    assert!(top_2_ids.iter().any(|id| original_ids.contains(&id.as_str())), "Should find test documents in top 2");

    let top_1_results = db.search_similar("test", &base_embedding, 1)
        .await
        .expect("Should limit to top 1 result");

    assert_eq!(top_1_results.len(), 1);
    // Should return one of our test documents
    assert!(original_ids.contains(&top_1_results[0].id.as_str()), "Should return a test document as top result");
    ...
    println!("Search performance: avg={:?}, min={:?}, max={:?}", avg_time, min_time, max_time);

    // Performance should be reasonable for in-memory operations
    assert!(max_time.as_millis() < 100, "Max search time should be under 100ms");
    assert!(avg_time.as_millis() < 50, "Average search time should be under 50ms");
