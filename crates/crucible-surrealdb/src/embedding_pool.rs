...
pub struct EmbeddingThreadPool {
    ...
    /// Signature of the embeddings generated by this pool
...
/// Metadata that identifies embeddings generated by a thread pool
#[derive(Debug, Clone, PartialEq)]
pub struct EmbeddingSignature {
    /// High-level provider type (fastembed, ollama, etc.)
    pub provider_type: EmbeddingProviderType,
    /// Human readable provider name (e.g. "FastEmbed", "Ollama")
    pub provider_name: String,
    /// Model identifier used when generating embeddings
    pub model: String,
...
impl EmbeddingSignature {
    /// Return provider type as a lowercase slug suitable for persistence
    pub fn provider_type_slug(&self) -> String {
...
impl EmbeddingThreadPool {
    ...
    pub async fn new_with_provider_config(
        ...
        let provider_name = embedding_provider
            .as_ref()
        ...
        let dimensions = embedding_provider
            .as_ref()
            .map(|p| p.dimensions())
            .unwrap_or_else(|| config.model_type.dimensions());

        let signature = EmbeddingSignature {
    ...
    /// Return the embedding signature for this pool
