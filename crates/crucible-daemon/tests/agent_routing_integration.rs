...
fn make_agent_manager() -> (AgentManager, Arc<SessionManager>, TempDir) {
    ...
    let agent_manager = AgentManager::new(session_manager.clone(), bg, None, None, None, None);
    (agent_manager, session_manager, tmp)
}

fn make_session_agent(agent_type: &str, agent_name: Option<&str>, provider: BackendType) -> SessionAgent {
    SessionAgent {
...
async fn unknown_agent_type_stores_config_without_error() {
    ...
    assert!(result.is_ok(), "configure_agent should succeed for unknown type");
    
    let retrieved = session_manager.get_session(&session.id).unwrap();
    assert!(retrieved.agent.is_some(), "Agent should be stored in session");
    assert_eq!(retrieved.agent.unwrap().agent_type, "unsupported_alien_type");
}
...
async fn unsupported_agent_type_fails_at_send_message_time() {
    ...
    let result = agent_manager
        .send_message(&session.id, "test message".to_string(), &event_tx)
        .await;
    
    assert!(result.is_err(), "send_message should fail for unsupported agent type");
...
async fn acp_agent_type_with_agent_name_stores_successfully() {
    ...
    assert!(result.is_ok(), "configure_agent should succeed for ACP with agent_name");
...
async fn internal_agent_type_with_mock_provider_stores_successfully() {
    ...
    assert!(result.is_ok(), "configure_agent should succeed for internal with Mock provider");
...
async fn internal_agent_type_with_ollama_stores_config() {
    ...
    assert!(result.is_ok(), "configure_agent should succeed for internal with Ollama");
...
async fn configure_agent_routing_decision_acp_vs_internal() {
    ...
    agent_manager.configure_agent(&session1.id, acp_agent).await.unwrap();
    
    let internal_agent = make_session_agent("internal", None, BackendType::Mock);
    agent_manager.configure_agent(&session2.id, internal_agent).await.unwrap();
