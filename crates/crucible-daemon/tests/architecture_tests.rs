...
/// Extract a function body (the text between its opening and matching closing
/// brace) given a substring that uniquely identifies the signature. Skips
/// braces that appear inside string literals and line comments so that
/// `json!({ ... })` and quoted `{}` don't unbalance the count.
...
//
// Adding a method is one row in CONFIG_METHODS.
// ===========================================================================
...
fn rpc_config_field_names_match_across_the_wire() {
    ...
    for m in CONFIG_METHODS {
        ...
        let server_set = fn_body(&server, &format!("fn handle_session_set_{}(", m.suffix));
        ...
        let server_get = fn_body(&server, &format!("fn handle_session_get_{}(", m.suffix));
        ...
        // Server result keys: `"field":` inside the response json.
        let mut server_res = captures(r#""([a-z_][a-z0-9_]*)"\s*:"#, &server_get);
