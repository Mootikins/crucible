...
impl CrucibleMcpServer {
    ...
    /// The tools this session may call, with their metadata.
    ///
    /// Per-session, not the full catalog: see [`Self::all_tool_names`] for the
    /// question "is this name ours".
    ///
    /// # Returns
    ///
    ...
    fn require_delegation(&self, tool: &str) -> Result<&DelegationContext, rmcp::ErrorData> {
        self.delegation_context.as_ref().ok_or_else(|| {
            rmcp::ErrorData::internal_error(
                format!("{tool} unavailable: no daemon delegation context"),
                None,
            )
        })
    }
...
impl CrucibleMcpServer {
    ...
    pub async fn delegate_session(
        ...
        let delegation = self.require_delegation("delegate_session")?;
        ...
        let content = if result.info.status == JobStatus::Completed {
            serde_json::json!({
                ...
                "child_session_id": spawned.child_session_id,
                "status": "completed",
                "result": crucible_core::text::truncate_bytes_marked(
