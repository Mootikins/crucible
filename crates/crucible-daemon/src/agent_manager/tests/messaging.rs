...
#[tokio::test]
async fn subagent_context_uses_session_workspace_not_kiln() {
    // Verify that SubagentContext.workspace is set to session.workspace,
    // not session.kiln. This is critical for delegation to work correctly.
    ...
    // Create a session with distinct kiln and workspace paths
    let session = session_manager
        .create_session(
            SessionType::Chat,
    ...
    // Create an agent config with delegation enabled
    let mut agent_config = test_agent();
    agent_config.delegation_config = Some(crucible_config::DelegationConfig {
        enabled: true,
        max_depth: 1,
    ...
    // Call setup_permission_handlers directly with the delegation-enabled config
    agent_manager.setup_permission_handlers(&session.id, &agent_config);

    // Verify the registered context has workspace set correctly
    let registered_context = agent_manager
        .background_manager
        .get_subagent_context(&session.id);

    assert!(
        registered_context.is_some(),
        "SubagentContext should be registered for session with delegation config"
    );

    let context = registered_context.unwrap();
    let expected_workspace = workspace_dir.path().to_path_buf();
    let kiln_path = kiln_dir.path().to_path_buf();

    assert_eq!(
        context.workspace, expected_workspace,
        "SubagentContext.workspace should equal session.workspace"
    );
    assert_ne!(
        context.workspace, kiln_path,
        "SubagentContext.workspace should NOT equal session.kiln"
    );
}
