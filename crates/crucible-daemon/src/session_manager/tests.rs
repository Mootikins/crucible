...
async fn test_create_session_no_workspace_gets_scratch_dir() {
    ...
    let manager = SessionManager::with_storage(storage)
        .with_session_workspace_dir(Some(scratch_base.clone()));
    let session = manager
        .create_session(SessionType::Chat, vec![kiln.clone()], None, None)
        .await
        .unwrap();
...
async fn test_create_session_scratch_dir_failure_falls_back_to_kiln() {
    ...
    let storage = temp_session_storage();
    let manager =
        SessionManager::with_storage(storage).with_session_workspace_dir(Some(scratch_base));
    let session = manager
        .create_session(SessionType::Chat, vec![kiln.clone()], None, None)
        .await
        .unwrap();
...
async fn test_list_sessions_filtered() {
    ...
    let manager = temp_session_manager();
    manager
        .create_session(SessionType::Chat, vec![kiln1.clone()], None, None)
        .await
        .unwrap();
    manager
        .create_session(SessionType::Agent, vec![kiln2.clone()], None, None)
        .await
        .unwrap();
    manager
        .create_session(SessionType::Chat, vec![kiln2.clone()], None, None)
        .await
        .unwrap();

    // Filter by kiln
    let filtered =
        manager.list_sessions_filtered(KilnFilter::Attached(&kiln1), None, None, None, true);
    assert_eq!(filtered.len(), 1);

    // Filter by type
    let filtered =
        manager.list_sessions_filtered(KilnFilter::Any, None, Some(SessionType::Chat), None, true);
...
async fn test_update_last_activity_updates_and_persists_timestamp() {
    ...
    let persisted = FileSessionStorage::new(manager.sessions_root().to_path_buf())
        .load(&session.id)
        .await
        .unwrap();
...
struct FailingSaveStorage(PathBuf);
...
async fn ending_a_session_outlasts_a_concurrent_last_activity_persist() {
    ...
    *storage.gate.lock().unwrap() = Some(gate);
    ...
    let persisted = manager.storage.load(&id).await.unwrap();
    assert_eq!(
        persisted.state,
        SessionState::Ended,
