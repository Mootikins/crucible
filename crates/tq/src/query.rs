...
pub fn compile_filter(code: &str) -> Result<CompiledFilter, TqError> {
    ...
    let modules = loader
        ...
        .map_err(|e| TqError::Compile(format!("{:?}", e)))?;
...
pub fn run_filter(filter: &CompiledFilter, input: Value) -> Result<Vec<Value>, TqError> {
        ...
        match unwrap_valr(output) {
            Ok(val) => {
                ...
                let json_val: Value = (&val)
                    .try_into()
                    .map_err(|_| TqError::Filter("Failed to convert output to JSON".to_string()))?;
                results.push(json_val);
            }
            Err(e) => {
                return Err(TqError::Filter(format!("{}", e)));
            }
        }
    }
