...
mod tests {
    ...
    fn test_tasks_pick_parses() {
        ...
        if let Some(Commands::Tasks { file: _, command }) = cli.command {
            assert!(matches!(command, crate::commands::tasks::TasksSubcommand::Pick { .. }));
        } else {
    ...
    fn test_tasks_done_parses() {
        ...
        if let Some(Commands::Tasks { file: _, command }) = cli.command {
            assert!(matches!(command, crate::commands::tasks::TasksSubcommand::Done { .. }));
        } else {
            panic!("Expected Tasks command");
        }
    }
    ...
    fn test_tasks_blocked_parses() {
        ...
        if let Some(Commands::Tasks { file: _, command }) = cli.command {
            assert!(matches!(command, crate::commands::tasks::TasksSubcommand::Blocked { .. }));
        } else {
            panic!("Expected Tasks command");
        }
    }
    ...
    fn test_tasks_blocked_with_reason_parses() {
        let cli = Cli::try_parse_from(["cru", "tasks", "blocked", "task-1", "waiting for review"]).unwrap();
        if let Some(Commands::Tasks { file: _, command }) = cli.command {
