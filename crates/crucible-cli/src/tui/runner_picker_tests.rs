...
fn test_vim_navigation_keys() {
    ...
    state.select_next();
    assert!(
        state.selected_index != initial_index,
        "j (next) should change selection"
    );
    ...
    assert_eq!(
        state.selected_index, initial_index,
        "k (prev) should return to initial"
    );
}
...
fn test_quick_select_numeric_keys() {
    ...
    assert!(
        state.agents.len() >= 2,
        "Need at least 2 agents for testing"
    );
...
fn test_confirm_requires_known_availability() {
    ...
    assert!(
        state.can_confirm(),
        "Should be able to confirm with known availability"
    );
}
...
fn test_internal_agent_always_available() {
    ...
    if let Some(agent) = internal {
        assert_eq!(
            agent.available,
            Some(true),
            "Internal agent should always be available"
        );
    }
}
...
fn test_all_confirm_keys_recognized() {
    ...
    let confirm_keys = vec![KeyCode::Enter, KeyCode::Char(' '), KeyCode::Char('l')];
...
fn test_all_cancel_keys_recognized() {
    ...
    let cancel_keys = vec![KeyCode::Esc, KeyCode::Char('q'), KeyCode::Char('h')];

    for key in cancel_keys {
        let is_cancel = matches!(key, KeyCode::Esc | KeyCode::Char('q') | KeyCode::Char('h'));
...
fn test_numeric_key_range() {
    ...
    for c in invalid {
        ...
        assert!(
            !is_numeric,
            "Char {} should NOT be recognized as numeric",
            c
        );
    }
}
...
fn test_all_acp_unavailable_allows_internal() {
    ...
    let internal_idx = state
        .agents
        .iter()
        .position(|a| a.name == "internal")
        .unwrap();
    ...
    assert!(
        state.can_confirm(),
        "Should be able to confirm internal when all ACP unavailable"
    );
}
...
fn test_can_confirm_false_when_unavailable() {
    ...
    assert!(
        !state.can_confirm(),
        "can_confirm should be false for unavailable agent"
    );
}
