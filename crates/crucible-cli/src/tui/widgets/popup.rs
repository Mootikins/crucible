...
// ─────────────────────────────────────────────────────────────────────────────
// GradientPopupRenderer (neovim-inspired raised effect)
// ─────────────────────────────────────────────────────────────────────────────

/// Dither characters for gradient fade effect (darkest to lightest)
/// Used to create "raised" effect on edges where popup overlaps background
const DITHER_CHARS: [char; 4] = ['▓', '▒', '░', ' '];

/// Ratatui widget for rendering a popup with edge-dither "floating" effect
///
/// This renderer creates a neovim-style popup that appears to float above
/// the background, using dither characters on the **horizontal edges** where
/// the popup content meets the background - creating a composited overlay look.
///
/// Layout concept:
/// ```text
/// bbbcccccccccccbbbbb  <- background padding, popup content, background padding
/// bbbcccccccccccbbbbb  <- more popup content with edge padding
/// dddcccccccccccddddd  <- dither on edges where popup meets background
/// fpfttttttttttttttbb  <- input directly below (no dither between popup and input)
/// ddddddddddddddddddd  <- dither below input (separate from popup)
/// ```
///
/// Features:
/// - No borders (flush with content)
/// - Dither on left/right edges (composited overlay effect)
/// - Popup content has full color, appears to overlap background
/// - Directly above input with no separator
pub struct GradientPopupRenderer<'a, T: PopupItem> {
    popup: &'a Popup<T>,
    style: PopupStyle,
    /// Width of dither margin on each side (default: 3)
    edge_margin: u16,
}

impl<'a, T: PopupItem> GradientPopupRenderer<'a, T> {
    /// Create a new gradient renderer for the given popup
    pub fn new(popup: &'a Popup<T>) -> Self {
        Self {
            popup,
            style: PopupStyle::default(),
    ...
    /// Set width of dither margin on each edge
    pub fn edge_margin(mut self, width: u16) -> Self {
        self.edge_margin = width;
        self
    }

    /// Build a line for a single item (same as PopupRenderer)
    fn build_item_line(
        &self,
        item: &T,
        is_selected: bool,
    ...
    /// Build dither string for edge margin
    fn build_edge_dither(&self, width: u16) -> String {
        // Use darkest dither char for edge effect
        DITHER_CHARS[0].to_string().repeat(width as usize)
    }
}

impl<T: PopupItem> Widget for GradientPopupRenderer<'_, T> {
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        // Build visible items
        let visible = self.popup.visible_items();
        let item_lines: Vec<Line> = visible
            .iter()
            .map(|(_, item, is_selected, is_multi)| {
                self.build_item_line(item, *is_selected, *is_multi)
            })
        ...
        // Render all rows with FULL COLOR (no dither - popup overlaps dithered input area)
        let content_count = item_lines.len();
        ...
        // Add scroll indicators on the right edge if needed
        if self.popup.has_items_above() {
            let indicator = Span::styled("↑", self.style.scroll_indicator);
            buf.set_span(area.x + area.width - 1, area.y, &indicator, 1);
        }
        if self.popup.has_items_below() {
            let last_y = area.y + content_count.saturating_sub(1) as u16;
            if last_y < area.y + area.height {
                let indicator = Span::styled("↓", self.style.scroll_indicator);
                buf.set_span(area.x + area.width - 1, last_y, &indicator, 1);
            }
        }
    }
}
