...
pub async fn execute(path: Option<PathBuf>, force: bool, interactive: bool) -> Result<()> {
    ...
    let (provider, model) = if interactive && !providers.is_empty() {
        // Interactive mode uses dialoguer
...
fn create_kiln_with_config(crucible_dir: &Path, config_content: &str, force: bool) -> Result<()> {
...
/// Interactive prompt for provider selection using dialoguer
...
/// Generate config with selected provider and model
...
mod tests {
    ...
    fn test_generate_config_endpoint_mapping() {
        ...
        // Unknown defaults to ollama endpoint
