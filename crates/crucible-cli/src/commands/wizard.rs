...
pub fn generate_initial_init_lua(provider: &str, default_kiln_path: &str) -> String {
    ...
    // The DIRECTORY names the kiln. A wizard that wrote `default = "~/vault"`
    // put a kiln called "default" in every picker the user ever opened, while
    // the directory beside it said `vault`; "default" is the name of the
    // pointer, never of a kiln.
    //
    // A path with no usable basename writes no entry and no pointer: the
    // config then names no kiln, and the last resort is the bundled help
    // corpus — a real directory — rather than an entry nothing resolves.
    if let Some(name) = crucible_core::config::synthesized_kiln_name(std::path::Path::new(
        default_kiln_path.trim_end_matches('/'),
    )) {
        value["default_kiln"] = serde_json::json!(name);
        value["kilns"] = serde_json::json!({ name: default_kiln_path });
    }
...
mod tests {
    ...
    /// The directory names the kiln. The wizard wrote `default` for both the
    /// pointer and the entry, so a brand-new user's very first picker showed a
    /// kiln called "default" next to a folder called something else.
    #[test]
    fn the_wizard_names_the_kiln_after_its_directory_and_never_default() {
        for (path, expected) in [
            ("~/notes", "notes"),
            ("~/My Vault", "My Vault"),
            ("/srv/Team Docs/", "Team Docs"),
        ] {
            let lua = generate_initial_init_lua("anthropic", path);
            let config =
                crucible_lua::evaluate_config_source(&lua).expect("the file must evaluate");

            assert_eq!(
                config.default_kiln.as_deref(),
                Some(expected),
                "{path} must be named after its directory"
            );
            assert!(
                config.kilns.contains_key(expected),
                "the pointer must name an entry that exists: {:?}",
                config.kilns
            );
            assert!(
                !config.kilns.contains_key("default"),
    ...
    /// A path with no usable basename writes no entry rather than one nothing
    /// can resolve. The default is then the bundled help corpus.
    #[test]
    fn the_wizard_writes_no_kiln_entry_for_a_path_that_names_nothing() {
        let lua = generate_initial_init_lua("anthropic", "/");
        let config = crucible_lua::evaluate_config_source(&lua).expect("the file must evaluate");

        assert!(config.kilns.is_empty(), "{:?}", config.kilns);
        assert_eq!(
            config.default_kiln, None,
            "a pointer with nothing to point at is worse than no pointer"
        );
    }
