...
use super::benchmarks::{
    print_benchmark_results, print_comparison_results, BenchmarkConfig, PerformanceBenchmarks,
};
...
pub async fn compare_caching_strategies(
    _tool_dir: PathBuf,
...
fn validate_performance(results: &super::benchmarks::BenchmarkResults) {
    ...
    if results.initialization_time_ms > 100 {
        warn!(
            "Registry initialization time is high: {}ms (target: <100ms)",
            results.initialization_time_ms
        );
    } else {
        info!(
            "✓ Registry initialization time is acceptable: {}ms",
            results.initialization_time_ms
        );
    }
    ...
    if results.summary.avg_discovery_time_ms > 50.0 {
        warn!(
            "Tool discovery time is high: {:.2}ms (target: <50ms)",
            results.summary.avg_discovery_time_ms
        );
    } else {
        info!(
            "✓ Tool discovery time is acceptable: {:.2}ms",
            results.summary.avg_discovery_time_ms
        );
    }
    ...
    if results.summary.avg_cache_hit_rate < 0.5 {
        warn!(
            "Cache hit rate is low: {:.2}% (target: >50%)",
            results.summary.avg_cache_hit_rate * 100.0
        );
    } else {
        info!(
            "✓ Cache hit rate is good: {:.2}%",
            results.summary.avg_cache_hit_rate * 100.0
        );
    }
    ...
    if results.summary.p95_execution_time_ms > results.summary.avg_execution_time_ms * 3.0 {
        warn!(
            "Execution times have high variance: avg={:.2}ms, p95={:.2}ms",
            results.summary.avg_execution_time_ms, results.summary.p95_execution_time_ms
        );
    } else {
        info!(
            "✓ Execution times are consistent: avg={:.2}ms, p95={:.2}ms",
            results.summary.avg_execution_time_ms, results.summary.p95_execution_time_ms
        );
    }
}
...
fn analyze_caching_improvements(
    results: &std::collections::HashMap<String, super::benchmarks::BenchmarkResults>,
) {
    ...
    if let (Some(no_cache), Some(default)) = (no_caching, default_caching) {
        let discovery_improvement = ((no_cache.summary.avg_discovery_time_ms
            - default.summary.avg_discovery_time_ms)
            / no_cache.summary.avg_discovery_time_ms)
            * 100.0;

        if discovery_improvement > 0.0 {
            info!(
                "✓ Default caching improves discovery by {:.1}%",
        ...
        if default.summary.avg_cache_hit_rate > no_cache.summary.avg_cache_hit_rate {
            info!(
                "✓ Default caching improves cache hit rate: {:.1}% vs {:.1}%",
                default.summary.avg_cache_hit_rate * 100.0,
                no_cache.summary.avg_cache_hit_rate * 100.0
            );
        }
    }

    if let (Some(default), Some(fast)) = (default_caching, fast_caching) {
        let discovery_improvement = ((default.summary.avg_discovery_time_ms
            - fast.summary.avg_discovery_time_ms)
            / default.summary.avg_discovery_time_ms)
            * 100.0;

        if discovery_improvement > 10.0 {
            info!(
                "✓ Fast caching significantly improves discovery by {:.1}%",
                discovery_improvement
            );
        }
    }
...
pub async fn test_lazy_loading(tool_dir: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Registry creation time: {}ms (should be fast due to lazy loading)",
        init_time.as_millis()
    );
    ...
    info!(
        "First tool listing time: {}ms (includes lazy initialization)",
        first_listing_time.as_millis()
    );
    ...
    info!(
        "Second tool listing time: {}ms (should be cached)",
        second_listing_time.as_millis()
    );

    // Analyze lazy loading effectiveness
    if first_listing_time.as_millis() > 50 {
        warn!(
            "Lazy initialization took longer than expected: {}ms",
            first_listing_time.as_millis()
        );
    } else {
        info!(
            "✓ Lazy initialization is efficient: {}ms",
            first_listing_time.as_millis()
        );
    }

    if second_listing_time.as_millis() < first_listing_time.as_millis() {
        let improvement = ((first_listing_time.as_millis() - second_listing_time.as_millis())
            as f64
            / first_listing_time.as_millis() as f64)
            * 100.0;
        info!(
            "✓ Caching improves subsequent listings by {:.1}%",
            improvement
        );
    }
...
pub async fn test_memory_usage(tool_dir: PathBuf) -> Result<(), Box<dyn std::error::Error>> {
    ...
    info!(
        "Memory after registry creation: {} bytes (+{})",
        after_registry_memory,
        after_registry_memory - baseline_memory
    );
    ...
    info!(
        "Memory after initialization: {} bytes (+{})",
        after_init_memory,
        after_init_memory - baseline_memory
    );
    ...
    info!(
        "Memory after operations: {} bytes (+{})",
        after_ops_memory,
        after_ops_memory - baseline_memory
    );

    // Check for memory leaks
    let memory_growth = after_ops_memory - after_init_memory;
    if memory_growth > 1_000_000 {
        // 1MB
        warn!(
            "Potential memory leak detected: {} bytes growth after operations",
            memory_growth
        );
    } else {
