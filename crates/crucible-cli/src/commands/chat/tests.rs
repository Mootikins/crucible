...
//
// The flag lives on the shared `Commands::Chat` variant, so interactive
// chat must honour it exactly as `cru chat -q` does. The asymmetry
...
fn absent_context_flags_send_no_precognition_rpc() {
    ...
    // `:set noprecognition`.
    assert!(precognition_flag_actions(false).is_empty());
}
...
fn interactive_initial_sets_reject_invalid_set_overrides() {
    let err = build_initial_sets(&["definitely_not_a_key=1".to_string()], false)
