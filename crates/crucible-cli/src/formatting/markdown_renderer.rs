...
pub fn render_markdown(text: &str) -> String {
    ...
    RenderContext::default().render_node(&ast)
}
...
impl RenderContext {
    ...
    fn render_node(&self, node: &Node) -> String {
        ...
        if node.is::<markdown_it::parser::core::Root>() {
            ...
            let inner_ctx = self.in_blockquote();
            for child in node.children.iter() {
                let child_text = inner_ctx.render_node(child);
            ...
            // Unordered list
            let inner_ctx = self.indented();
            for child in node.children.iter() {
                if child.cast::<ListItem>().is_some() {
                    let item_text = Self::render_inline_children(child);
            ...
            output.push_str(&Self::render_table(node));
        } else {
    ...
    fn render_inline_children(node: &Node) -> String {
        let mut output = String::new();
        for child in node.children.iter() {
            output.push_str(&Self::render_inline(child));
        }
        output
    }
    ...
    fn render_table(node: &Node) -> String {
                    ...
                    if row_node.cast::<TableRow>().is_some() {
                        ...
                        for cell_node in row_node.children.iter() {
                            if cell_node.cast::<TableCell>().is_some() {
                                let cell_text = Self::render_inline_children(cell_node);
        ...
        output.push_str(&format!("{}\n", Self::style_border(&bottom_border)));
