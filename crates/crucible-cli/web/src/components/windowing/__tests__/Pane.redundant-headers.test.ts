...
describe('Pane.tsx - Redundant Headers Removal', () => {
  ...
  it('file case should NOT contain header div with tab.title', () => {
    ...
    expect(fileCase).not.toMatch(/<span class="text-sm text-shell-body">\{tab\.title\}<\/span>/);
  });
  ...
  it('preview case should NOT contain paragraph with tab.title', () => {
    ...
    expect(previewCase).not.toMatch(/<p class="text-sm text-muted mt-3 text-center">\{tab\.title\}<\/p>/);
  });
  ...
  it('tool case should NOT contain "Tool: {tab.title}" div', () => {
    ...
    expect(toolCase).not.toMatch(/<div class="text-shell-body text-sm">Tool: \{tab\.title\}<\/div>/);
  });
