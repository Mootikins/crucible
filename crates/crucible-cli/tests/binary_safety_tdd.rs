//! TDD Tests for Binary File Detection and Memory Protection
//!
//! This test suite implements Test-Driven Development for critical safety features
//! in the search functionality. The tests follow the RED-GREEN-REFACTOR cycle:
//!
//! RED PHASE: Tests are written to FAIL, demonstrating missing safety features
//! GREEN PHASE: Minimal implementation is added to make tests pass
//! REFACTOR PHASE: Code is improved while maintaining test coverage
//!
...
impl BinarySafetyTestHarness {
    ...
    pub fn create_binary_file(&self, relative_path: &str, content: &[u8]) -> Result<String> {
        let full_path = self.test_vault.vault_path.join(relative_path);
    ...
    pub fn create_mixed_content_file(&self, relative_path: &str, content: &[u8]) -> Result<String> {
        self.create_binary_file(relative_path, content)
    }

    /// Get the vault path for testing
    pub fn vault_path(&self) -> &Path {
        &self.test_vault.vault_path
    }
}
...
mod binary_detection_tests {
    ...
    fn test_detect_mixed_binary_text_content() -> Result<()> {
        ...
        let file_path = harness.create_mixed_content_file("mixed.md", mixed_content)?;
...
mod memory_protection_tests {
    ...
    #[test]
    fn test_graceful_binary_detection_failure() -> Result<()> {
        let harness = BinarySafetyTestHarness::new()?;

        // Create content that might confuse binary detection
        let tricky_content = b"# Valid Header\n\xFF\xFE\xFD\xFC\xFB\xFA\n# More content\n";

        let file_path = harness.create_binary_file("tricky.md", tricky_content)?;

        let result = get_file_content(&file_path);

        // Should handle gracefully - either detect as binary or attempt UTF-8 recovery
        // but should not crash or panic
        match result {
            Ok(_) => {
                // If it succeeds, it should be through proper UTF-8 recovery
            }
            Err(e) => {
                let error_msg = e.to_string().to_lowercase();
    ...
    /// Test multiple large files don't cause memory exhaustion
    #[test]
    fn test_multiple_large_files_memory_safety() -> Result<()> {
        let harness = BinarySafetyTestHarness::new()?;

        // Create multiple large files
        for i in 0..5 {
            let content = format!("# Document {}\n{}\n", i, "Large content block. ".repeat(100000));
            harness.create_text_file(&format!("large_{}.md", i), &content)?;
        }

        // Search across all files - should not cause memory exhaustion
        let results = search_files_in_kiln(harness.vault_path(), "content", 100, false)?;

        // Should complete without crashing
        assert!(results.len() <= 100, "Should respect result limit");

        // Verify all results are properly processed
        for result in results {
            assert!(!result.content.is_empty(), "Each result should have content");
            assert!(result.content.len() <= 1024 * 1024, "Content should be within memory limits");
        }
...
mod file_size_boundary_tests {
    ...
    fn test_file_exactly_at_limit() -> Result<()> {
        ...
        assert_eq!(metadata.len(), 10 * 1024 * 1024, "File should be exactly 10MB");
...
mod integration_tests {
    ...
    #[test]
    fn test_memory_usage_large_search() -> Result<()> {
        let harness = BinarySafetyTestHarness::new()?;

        // Create many files with substantial content
        for i in 0..20 {
            let content = format!("# Document {}\n", i) + &"Large content block with searchable terms. ".repeat(5000);
            harness.create_text_file(&format!("doc_{}.md", i), &content)?;
        }

        // Search should complete without memory issues
        let results = search_files_in_kiln(harness.vault_path(), "searchable", 50, false)?;

        assert_eq!(results.len(), 20, "Should find all documents");

        // Verify all results have reasonable content sizes
        for result in results {
            assert!(result.content.len() <= 1024 * 1024, "Each result content should be under 1MB");
        }
