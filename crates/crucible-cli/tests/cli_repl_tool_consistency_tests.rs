...
mod tool_comparison {
    ...
    pub fn compare_tool_lists(cli_tools: &[String], repl_tools: &[String]) -> ToolComparisonResult {
        ...
        let missing_in_repl: Vec<String> = cli_set.difference(&repl_set).map(|s| (*s).clone()).collect();
        let missing_in_cli: Vec<String> = repl_set.difference(&cli_set).map(|s| (*s).clone()).collect();
        let common: Vec<String> = cli_set.intersection(&repl_set).map(|s| (*s).clone()).collect();
...
mod execution_consistency {
        ...
        for tool_name in test_tools {
            ...
            match consistency {
                ExecutionConsistency::Consistent => {
                    results.insert(tool_name.clone(), ExecutionTestResult {
    ...
    async fn test_tool_via_repl(tool_name: &str, registry: &UnifiedToolRegistry) -> crucible_tools::ToolResult {
                ...
                match result.status {
                    crucible_cli::commands::repl::tools::ToolStatus::Success => {
                        crucible_tools::ToolResult {
                            success: true,
                            data: Some(serde_json::from_str(&result.output).unwrap_or(json!(result.output))),
                            error: None,
                            duration_ms: 0,
            ...
            match (&cli_result.data, &repl_result.data) {
                (Some(cli_data), Some(repl_data)) => {
                    ...
                    if are_results_similar(cli_data, repl_data) {
                        ...
                        ExecutionConsistency::Inconsistent(
                            format!("Data differs: CLI={:?}, REPL={:?}", cli_data, repl_data)
                        )
                    }
                }
                (Some(_), None) => ExecutionConsistency::Inconsistent(
                    "CLI has data but REPL doesn't".to_string()
                ),
                (None, Some(_)) => ExecutionConsistency::Inconsistent(
                    "REPL has data but CLI doesn't".to_string()
                ),
            ...
            match (&cli_result.error, &repl_result.error) {
                (Some(cli_err), Some(repl_err)) => {
                    if are_errors_similar(cli_err, repl_err) {
                        ...
                        ExecutionConsistency::Inconsistent(
                            format!("Different errors: CLI={}, REPL={}", cli_err, repl_err)
                        )
                    }
                }
    ...
    fn are_errors_similar(cli_err: &str, repl_err: &str) -> bool {
        ...
        let common_words: usize = cli_words.iter()
...
mod error_handling_consistency {
    ...
    pub async fn test_error_handling_consistency(registry: &UnifiedToolRegistry) -> ErrorHandlingResult {
        let mut results = HashMap::new();
        let mut consistent_count = 0;
        ...
        let error_scenarios = vec![
            ...
            ("invalid_params", vec!["system_info".to_string(), "invalid_param".to_string(), "another_param".to_string()]),
        ...
        for (scenario_name, args) in error_scenarios {
            ...
            results.insert(scenario_name.to_string(), ErrorTestResult {
    ...
    async fn test_cli_error_handling(args: &[String]) -> String {
        let tool_name = if args.is_empty() { "nonexistent_tool" } else { &args[0] };
    ...
    fn are_error_responses_consistent(cli_error: &str, repl_error: &str) -> bool {
        ...
        let cli_has_indicator = error_indicators.iter().any(|&indicator| cli_norm.contains(indicator));
        let repl_has_indicator = error_indicators.iter().any(|&indicator| repl_norm.contains(indicator));
...
mod tests {
    ...
    async fn test_cli_repl_tool_discovery_consistency() -> Result<()> {
        ...
        assert!(cli_tools.len() >= 20, "CLI should have at least 20 tools, found {}", cli_tools.len());
        assert!(repl_tools.len() >= 20, "REPL should have at least 20 tools, found {}", repl_tools.len());
        ...
        for expected_tool in expected_tools {
            assert!(
                ...
                "CLI should have {} tool", expected_tool
            );
            assert!(
                repl_tools.contains(&expected_tool.to_string()),
                "REPL should have {} tool", expected_tool
            );
        }
        ...
        if !comparison.is_identical {
            ...
            let total_difference = comparison.missing_in_repl.len() + comparison.missing_in_cli.len();
            assert!(
                total_difference <= 5,
        ...
        println!("\n✅ CLI/REPL tool discovery consistency test passed in {}ms", duration.as_millis());
    ...
    async fn test_cli_repl_execution_consistency() -> Result<()> {
        ...
        println!("\n1️⃣ Testing execution consistency on {} common tools...", comparison.common.len());
        ...
        // Assertions
        assert!(execution_result.total_tested > 0, "Should test at least one tool");

        let consistency_rate = if execution_result.total_tested > 0 {
            (execution_result.consistent_count as f64 / execution_result.total_tested as f64) * 100.0
        } else {
            0.0
        };
        ...
        let duration = start_time.elapsed();
        println!("\n✅ CLI/REPL execution consistency test passed in {}ms", duration.as_millis());
    ...
    async fn test_cli_repl_error_handling_consistency() -> Result<()> {
        ...
        // Assertions
        assert!(error_result.total_tests > 0, "Should test at least one error scenario");
        ...
        let duration = start_time.elapsed();
        println!("\n✅ CLI/REPL error handling consistency test passed in {}ms", duration.as_millis());
    ...
    async fn test_comprehensive_cli_repl_integration() -> Result<()> {
        ...
        let execution_result = test_execution_consistency(&discovery_comparison.common, &registry).await;
        execution_result.print_summary();
        ...
        let discovery_score = if discovery_comparison.is_identical { 100.0 } else { 85.0 };
        ...
        let duration = start_time.elapsed();
        println!("\n✅ Comprehensive CLI/REPL integration test passed in {}ms", duration.as_millis());
...
mod performance_comparison {
    ...
    impl PerformanceResult {
        pub fn print_summary(&self) {
            ...
            if self.cli_discovery_time_ms > 0 && self.repl_discovery_time_ms > 0 {
                let diff = (self.cli_discovery_time_ms as isize - self.repl_discovery_time_ms as isize).abs();
                ...
                if percent_diff <= 50.0 {
                    ...
                    println!("    ⚠️  Discovery times differ significantly: {:.1}%", percent_diff);
                }
            }
            ...
            if !self.cli_execution_times_ms.is_empty() && !self.repl_execution_times_ms.is_empty() {
                ...
                if count > 0 {
                    ...
                    if percent_diff <= 100.0 {
                        ...
                        println!("    ⚠️  Execution times differ significantly: {:.1}%", percent_diff);
                    }
                }
            }
        }
    ...
    pub async fn test_interface_performance(
        ...
        let test_tools = if cli_tools.len() > 3 { &cli_tools[..3] } else { cli_tools };
        ...
        for tool_name in test_tools {
            ...
            result.cli_execution_times_ms.insert(tool_name.clone(), start.elapsed().as_millis() as u64);

            // Test REPL execution time
            let start = std::time::Instant::now();
            let _ = registry.execute_tool(tool_name, &[]).await;
            result.repl_execution_times_ms.insert(tool_name.clone(), start.elapsed().as_millis() as u64);
        }
