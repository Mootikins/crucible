...
use crucible_cli::{
    ...
    error_recovery::{
        CircuitBreakerConfig, CircuitBreaker, ErrorRecoveryManager,
        RetryConfig, SearchFallbackManager, SearchStrategy, ServiceHealthMonitor,
        retry_with_backoff, is_embedding_error_retryable, get_embedding_error_retry_delay,
    },
};
    ...
    for attempt in 1..=10 {
        if circuit_breaker.is_request_allowed().await {
            ...
            if attempt <= 5 {
                circuit_breaker.record_failure().await;
                failure_count += 1;
                println!("Attempt {}: Operation failed (failure #{})", attempt, failure_count);
            } else {
...
async fn test_service_health_monitoring() {
    ...
    // Initially unknown
    assert_eq!(health_monitor.get_health("test_service").await,
               crucible_cli::error_recovery::ServiceHealth::Unknown);

    // Update health
    health_monitor.update_health("test_service",
        crucible_cli::error_recovery::ServiceHealth::Healthy).await;

    assert!(health_monitor.is_healthy("test_service").await);

    // Test multiple services
    health_monitor.update_health("service_a",
        crucible_cli::error_recovery::ServiceHealth::Healthy).await;
    health_monitor.update_health("service_b",
        crucible_cli::error_recovery::ServiceHealth::Degraded).await;
    health_monitor.update_health("service_c",
        crucible_cli::error_recovery::ServiceHealth::Unhealthy).await;
...
async fn test_search_fallback_with_service_failures() {
    ...
    health_monitor.update_health("embedding_service",
        crucible_cli::error_recovery::ServiceHealth::Unhealthy).await;
...
async fn test_embedding_error_recovery() {
    ...
    for error in retryable_errors {
        assert!(is_embedding_error_retryable(&error),
        ...
        assert!(delay.unwrap() > Duration::from_millis(0),
               "Delay should be positive: {:?}", delay);
    }
...
async fn test_semantic_search_error_handling() {
    ...
    let result = timeout(Duration::from_secs(10), semantic::execute(
        config,
...
async fn test_end_to_end_error_recovery_workflow() {
    ...
    let error_recovery_manager = ErrorRecoveryManager::new(&create_test_config());
...
async fn test_error_recovery_performance() {
    ...
    assert!(elapsed < Duration::from_millis(100),
           "Error recovery operations should be fast, got {:?}", elapsed);
}
...
async fn test_error_recovery_comprehensive_validation() {
    ...
    let health_monitor = ServiceHealthMonitor::new();
    health_monitor.update_health("test",
        crucible_cli::error_recovery::ServiceHealth::Healthy).await;
