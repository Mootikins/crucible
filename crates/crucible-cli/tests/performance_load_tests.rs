...
impl PerformanceTester {
    ...
    pub async fn run_service_performance_test(&self, context: &TestContext) -> Result<PerformanceTestResults> {
        println!("Running service performance test with {} operations...", self.config.iterations);
        ...
        for i in 0..self.config.iterations {
            ...
            let result = timeout(self.config.timeout_duration, service_execute(context.config.clone(), command)).await;
        ...
        let memory_peak = memory_samples.into_iter()
        ...
        let results = PerformanceTestResults {
            ...
            p50_response_time: response_times.get(p50_index).cloned().unwrap_or(Duration::ZERO),
            p95_response_time: response_times.get(p95_index).cloned().unwrap_or(Duration::ZERO),
            p99_response_time: response_times.get(p99_index).cloned().unwrap_or(Duration::ZERO),
        ...
        println!("  Operations: {}/{} ({:.1}% success)",
                results.successful_operations, results.total_operations, results.success_rate());
        println!("  Throughput: {:.2} ops/sec", results.throughput_ops_per_second);
        println!("  Response times: p50={:?}, p95={:?}, p99={:?}",
                results.p50_response_time, results.p95_response_time, results.p99_response_time);
        println!("  Memory growth: {} MB",
                (results.memory_usage_after.rss_bytes - results.memory_usage_before.rss_bytes) / 1024 / 1024);

        Ok(results)
    }

    pub async fn run_migration_performance_test(&self, context: &TestContext) -> Result<PerformanceTestResults> {
        println!("Running migration performance test with {} operations...", self.config.iterations);
        ...
        for i in 0..self.config.iterations {
            ...
            let result = timeout(self.config.timeout_duration, migration_execute(context.config.clone(), command)).await;
    ...
    pub async fn run_rune_performance_test(&self, context: &TestContext) -> Result<PerformanceTestResults> {
        println!("Running Rune performance test with {} operations...", self.config.iterations);
        ...
        // Create test scripts
        let scripts = vec![
            ("simple", r#"function main(args) { return { success: true }; }"#),
            ("with_args", r#"function main(args) { return { success: true, args: args }; }"#),
            ("complex", r#"
                function main(args) {
        ...
        for _ in 0..self.config.warmup_iterations {
            ...
            let _ = rune_execute(context.config.clone(), script_path.to_string_lossy().to_string(), None).await;
        }
        ...
        for i in 0..self.config.iterations {
            ...
            let result = timeout(self.config.timeout_duration, rune_execute(
    ...
    pub async fn run_concurrent_load_test(&self, context: &TestContext) -> Result<PerformanceTestResults> {
        println!("Running concurrent load test with {} operations over {} concurrent workers...",
        ...
        for worker_id in 0..self.config.concurrent_operations {
            ...
            let future = async move {
                ...
                for i in 0..operations {
                    ...
                    let result = timeout(timeout_duration, service_execute(config.clone(), command)).await;
                    let response_time = operation_start.elapsed();
...
async fn test_service_command_performance() -> Result<()> {
    ...
    assert!(results.success_rate() >= 95.0, "Success rate should be at least 95%");
    assert!(results.average_response_time < Duration::from_millis(200), "Average response time should be under 200ms");
    assert!(results.throughput_ops_per_second >= 5.0, "Throughput should be at least 5 ops/sec");
...
async fn test_migration_command_performance() -> Result<()> {
    ...
    assert!(results.success_rate() >= 90.0, "Migration success rate should be at least 90%");
    assert!(results.average_response_time < Duration::from_millis(500), "Migration commands should be reasonably fast");
    assert!(results.throughput_ops_per_second >= 2.0, "Migration throughput should be at least 2 ops/sec");
...
async fn test_rune_command_performance() -> Result<()> {
    ...
    assert!(results.success_rate() >= 85.0, "Rune success rate should be at least 85%");
    assert!(results.average_response_time < Duration::from_millis(1000), "Rune execution should be reasonably fast");
    assert!(results.throughput_ops_per_second >= 1.0, "Rune throughput should be at least 1 ops/sec");
...
async fn test_concurrent_load_performance() -> Result<()> {
    ...
    assert!(results.success_rate() >= 90.0, "Concurrent success rate should be at least 90%");
    assert!(results.throughput_ops_per_second >= 10.0, "Concurrent throughput should be high");
    assert!(results.p95_response_time < Duration::from_secs(2), "95th percentile should be reasonable");
...
async fn test_memory_leak_detection() -> Result<()> {
    ...
    for round in 0..5 {
        ...
        // Run Rune commands
        for i in 0..5 {
            let script_path = context.create_test_script(&format!("memory-test-{}-{}", round, i),
        ...
        let memory_growth = current_memory.rss_bytes.saturating_sub(initial_memory.rss_bytes);

        println!("  Memory growth: {} MB", memory_growth / 1024 / 1024);

        // Memory growth should be reasonable
        assert!(memory_growth < 100 * 1024 * 1024, "Memory growth should be under 100MB per round");
    }

    let final_memory = MemoryUsage::current();
    let total_growth = final_memory.rss_bytes.saturating_sub(initial_memory.rss_bytes);

    println!("Total memory growth: {} MB", total_growth / 1024 / 1024);
    assert!(total_growth < 200 * 1024 * 1024, "Total memory growth should be under 200MB");
...
async fn test_scalability_limits() -> Result<()> {
    ...
    for concurrent_ops in concurrent_levels {
        println!("Testing scalability with {} concurrent operations", concurrent_ops);
    ...
    println!("Optimal concurrency level: {} (throughput: {:.2} ops/sec)", optimal_concurrency, max_throughput);

    // Scalability assertions
    assert!(optimal_concurrency >= 2, "Should benefit from some concurrency");
    assert!(max_throughput >= 10.0, "Should achieve reasonable throughput");
...
async fn test_resource_cleanup() -> Result<()> {
    ...
    let temp_services: Vec<String> = (0..10).map(|i| format!("temp-service-{}", i)).collect();

    for service_name in &temp_services {
        let service = Arc::new(MockService::new(service_name));
        mock_services.add_service(service_name.clone(), service).await;
        ...
        let _ = service_execute(context.config.clone(), start_command).await;
        ...
        for _ in 0..5 {
            ...
            let _ = service_execute(context.config.clone(), health_command).await;
        }
        ...
        let _ = service_execute(context.config.clone(), stop_command).await;
    }

    // Check memory usage after operations
    let post_ops_memory = MemoryUsage::current();
    let memory_growth = post_ops_memory.rss_bytes.saturating_sub(initial_memory.rss_bytes);

    println!("Memory growth after service operations: {} MB", memory_growth / 1024 / 1024);

    // Simulate cleanup (in real implementation, this would be actual resource cleanup)
    tokio::time::sleep(Duration::from_millis(100)).await;

    // Check final memory usage
    let final_memory = MemoryUsage::current();
    let final_growth = final_memory.rss_bytes.saturating_sub(initial_memory.rss_bytes);

    println!("Final memory growth: {} MB", final_growth / 1024 / 1024);

    // Resource cleanup assertions
    assert!(memory_growth < 50 * 1024 * 1024, "Memory growth should be reasonable after service operations");
    assert!(final_growth <= memory_growth + 10 * 1024 * 1024, "Final memory should not grow significantly after cleanup");
...
async fn test_performance_regression_detection() -> Result<()> {
    ...
    println!("  Current throughput: {:.2} ops/sec (baseline: {:.2})",
            results.throughput_ops_per_second, baseline_throughput);
    println!("  Current p95: {:?} (baseline: {:?})",
            results.p95_response_time, baseline_p95_response);
    println!("  Current success rate: {:.1}% (baseline: {:.1}%)",
            results.success_rate(), baseline_success_rate);

    // Check for performance regressions
    let throughput_regression = (baseline_throughput - results.throughput_ops_per_second) / baseline_throughput > 0.20;
