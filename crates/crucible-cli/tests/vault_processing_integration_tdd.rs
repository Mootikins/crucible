...
use crucible_cli::config::{EmbeddingConfig as CliEmbeddingConfig, ProviderType};

/// Test vault path using the existing comprehensive test vault
const TEST_VAULT_PATH: &str = "./crucible/tests/test-kiln";
...
impl VaultProcessingTestContext {
    ...
    pub async fn new() -> Result<Self> {
        ...
        original_env_vars.insert("OBSIDIAN_KILN_PATH".to_string(), std::env::var("OBSIDIAN_KILN_PATH").ok());
        original_env_vars.insert("EMBEDDING_ENDPOINT".to_string(), std::env::var("EMBEDDING_ENDPOINT").ok());
        original_env_vars.insert("EMBEDDING_MODEL".to_string(), std::env::var("EMBEDDING_MODEL").ok());
        original_env_vars.insert("CRUCIBLE_TEST_MODE".to_string(), std::env::var("CRUCIBLE_TEST_MODE").ok());
        ...
        let temp_db_path = std::env::temp_dir().join(format!("crucible_vault_tdd_{}",
            std::process::id()));
    ...
    pub fn set_test_env_vars(&mut self, embedding_url: Option<&str>, embedding_model: Option<&str>) {
    ...
    pub fn create_test_cli_config(&self, embedding_url: &str, embedding_model: &str) -> Result<CliConfig> {
...
async fn test_vault_processing_uses_cli_embedding_configuration() {
    ...
    let mut test_ctx = VaultProcessingTestContext::new().await.expect("Failed to create test context");
    ...
    let cli_config = test_ctx.create_test_cli_config(custom_embedding_url, custom_embedding_model)
    ...
    let process_result = process_vault_files(
        ...
        &scanner_config,
        Some(&embedding_pool),
    ).await.expect("Failed to process vault files");
    ...
    let db_stats = get_database_stats(&test_ctx.client).await
        .expect("Failed to get database stats");

    // Verify embeddings were generated
    assert!(db_stats.total_embeddings > 0, "No embeddings were generated during vault processing");
...
async fn test_vault_processing_generates_real_embeddings() {
    ...
    assert!(db_stats.total_embeddings > 0, "No embeddings were generated");
    ...
    let sample_embeddings = test_ctx.client.query(
        "SELECT id, vector FROM embeddings LIMIT 1",
        &[]
    ).await.expect("Failed to query embeddings");

    assert!(!sample_embeddings.records.is_empty(), "No embeddings found to inspect");
...
async fn test_vault_processing_without_external_daemon() {
    ...
    assert!(process_result.processed_count > 0, "No files were processed");
    assert_eq!(process_result.processed_count, test_files.len(),
               "Processed file count mismatch. Expected: {}, Actual: {}",
               test_files.len(), process_result.processed_count);

    // Verify embeddings were created in the database
    let db_stats = get_database_stats(&test_ctx.client).await
        .expect("Failed to get database stats");

    assert!(db_stats.total_embeddings > 0, "No embeddings were created during processing");
    assert!(db_stats.total_embeddings >= process_result.processed_count as u64,
    ...
    assert!(processing_time < Duration::from_secs(30),
            "Processing took too long: {:?}. This may indicate external dependencies.", processing_time);

    // Verify database contains the processed documents
    let document_count = test_ctx.client.query("SELECT count() as count FROM documents", &[]).await
        .expect("Failed to query documents");

    let doc_count = document_count.records.first()
        .and_then(|r| r.data.get("count"))
        .and_then(|v| v.as_u64())
        .unwrap_or(0);

    assert!(doc_count >= process_result.processed_count as u64,
    ...
    println!("   📁 Processed {} files in {:?}", process_result.processed_count, processing_time);
...
async fn test_cli_embedding_configuration_conversion() {
    ...
    let test_ctx = VaultProcessingTestContext::new().await.expect("Failed to create test context");

    let custom_url = "https://custom-embedding.example.com:8080";
    let custom_model = "custom-embedding-v3";
    let cli_config = test_ctx.create_test_cli_config(custom_url, custom_model)
    ...
    assert!(embedding_config_result.is_ok(),
    ...
    assert_eq!(embedding_config.endpoint, custom_url,
               "Embedding config endpoint doesn't match CLI setting");
    assert_eq!(embedding_config.model, custom_model,
