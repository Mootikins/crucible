...
async fn test_system_tool_group_initialization_failures() -> Result<()> {
    ...
    let invalid_cache_config = ToolGroupCacheConfig {
        discovery_ttl: Duration::from_secs(-1), // Invalid TTL
        schema_ttl: Duration::from_secs(-1),    // Invalid TTL
        max_schema_cache_size: 0,
        caching_enabled: true,
    };

    let system_group = SystemToolGroup::with_cache_config(invalid_cache_config);

    // This should either initialize successfully (ignoring invalid config)
    // or fail gracefully with a descriptive error
    let mut group_clone = system_group;
    let init_result = group_clone.initialize().await;
    ...
    let mut group_clone = system_group;
    let init1 = group_clone.initialize().await;
    let init2 = group_clone.initialize().await; // Should be idempotent
...
async fn test_cache_invalidation_scenarios() -> Result<()> {
    ...
    let registry = UnifiedToolRegistry::with_fast_cache(context.tool_dir).await?;
...
async fn run_all_error_handling_tests() -> Result<()> {
    ...
    let test_results = vec![
        ("Missing Tools", test_missing_tool_comprehensive_error().await),
        ("Tool Group Lookup", test_missing_tool_group_lookup().await),
        ("Missing Tool Schema", test_missing_tool_schema_error().await),
        ("Parameter Conversion", test_parameter_conversion_failures().await),
        ("Parameter Validation", test_parameter_validation_failures().await),
        ("Tool Execution", test_tool_execution_failures().await),
        ("Backend Failures", test_crucible_tools_backend_failures().await),
        ("Group Initialization", test_tool_group_initialization_failures().await),
        ("SystemToolGroup Init", test_system_tool_group_initialization_failures().await),
        ("Cache Invalidation", test_cache_invalidation_scenarios().await),
        ("Cache Corruption", test_cache_corruption_scenarios().await),
        ("Schema Cache Edge Cases", test_schema_cache_edge_cases().await),
        ("System Recovery", test_system_recovery_after_errors().await),
        ("Concurrent Access", test_concurrent_access_error_handling().await),
        ("Memory Leak Prevention", test_memory_leak_prevention_in_errors().await),
    ];
