...
impl ReplProcess {
    ...
    pub fn spawn(vault_path: &str, db_path: Option<&str>) -> Result<Self> {
        ...
        let mut child = cmd.spawn()
            .context("Failed to spawn REPL process")?;
    ...
    pub fn wait_for_output(&self, pattern: &str, timeout_secs: u64) -> Result<bool> {
        ...
        warn!("Pattern '{}' not found in output after {} seconds", pattern, timeout_secs);
        Ok(false)
    }
    ...
    pub fn get_errors(&self) -> Result<String> {
        let buffer = self.error_buffer.lock()
    ...
    pub fn kill(&mut self) -> Result<()> {
        ...
        self.child.kill()
            .context("Failed to kill REPL process")?;
        Ok(())
    }
}
...
async fn test_repl_run_system_info() -> Result<()> {
    ...
    let found = repl.wait_for_output("OS:", 15)
    ...
    assert!(output.contains("OS:") || output.contains("os:"),
            "Output should contain OS information");
    assert!(output.contains("Memory:") || output.contains("memory:"),
            "Output should contain memory information");
    assert!(output.contains("Disk:") || output.contains("disk:"),
...
async fn test_repl_run_invalid_tool() -> Result<()> {
    ...
    let found = repl.wait_for_output("❌ Tool Error", 10)
...
async fn test_repl_multiple_tool_sequence() -> Result<()> {
    ...
    let search_found = repl.wait_for_output("results", 15)
...
async fn test_repl_stability_after_commands() -> Result<()> {
    ...
    assert!(help_found, "REPL should still be responsive after multiple commands");
...
async fn test_repl_run_tool_with_args() -> Result<()> {
    ...
    let found = repl.wait_for_output("results", 15)
        .or_else(|_| repl.wait_for_output("found", 15))
...
async fn test_repl_tool_execution_performance() -> Result<()> {
    ...
    assert!(execution_time < Duration::from_secs(20),
            "System info should complete within 20 seconds, took {:?}", execution_time);
