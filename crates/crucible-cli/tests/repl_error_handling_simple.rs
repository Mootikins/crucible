...
async fn test_missing_tool_error_handling() -> Result<()> {
    ...
    assert!(error_msg.contains("not found"), "Error should mention tool not found: {}", error_msg);
    assert!(error_msg.contains("definitely_nonexistent_tool_xyz"), "Error should include tool name: {}", error_msg);
...
async fn test_tool_execution_failures() -> Result<()> {
    ...
    match result {
        ...
        Err(e) => {
            ...
            assert!(error_msg.contains("failed") || error_msg.contains("error") || error_msg.contains("not found") || error_msg.contains("parameter"),
    ...
    let result = registry.execute_tool("read_file", &["/nonexistent/file/path/xyz.txt".to_string()]).await;
