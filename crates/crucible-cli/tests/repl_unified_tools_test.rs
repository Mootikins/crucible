...
async fn test_unified_registry_discovers_system_tools() -> Result<()> {
    ...
    println!("✅ UnifiedRegistry successfully discovered {} system tools", system_tools.len());
    ...
    assert!(result.output.contains("platform"), "Output should contain system info");
...
async fn test_unified_registry_tool_execution() -> Result<()> {
    ...
    assert!(result.is_success(), "get_environment should succeed with no args");
    println!("✅ get_environment: {} chars", result.output.len());

    // Test single argument tool
    let result = registry.execute_tool("list_files", &["/tmp".to_string()]).await?;
    assert!(result.is_success(), "list_files should succeed with path argument");
    println!("✅ list_files /tmp: {}", result.output);

    // Test multi-argument tool
    let result = registry.execute_tool("search_by_tags", &["tag1".to_string(), "tag2".to_string()]).await?;
    ...
    assert!(result.is_err(), "list_files should fail with missing arguments");
