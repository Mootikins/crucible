...
use crucible_cli::commands::repl::input::Input;
...
async fn test_repl_command_parsing_and_execution() -> Result<()> {
    ...
    assert!(matches!(parsed_tools_command, Input::Command(Command::ListTools)));
...
async fn test_unified_tool_registry_with_repl() -> Result<()> {
    ...
    assert!(grouped_tools.contains_key("system"), "Should have system group");
    ...
    if system_tools.contains(&"system_info".to_string()) {
        ...
        assert!(result.is_success(), "system_info should execute successfully");
        ...
        println!("✅ system_info executed successfully: {} chars", result.output.len());
    }

    // Test tool group lookup
    for tool_name in system_tools.iter().take(5) {
        let group = registry.get_tool_group(tool_name);
        assert_eq!(group.unwrap(), "system", "Tool {} should belong to system group", tool_name);
    }
...
async fn test_repl_tools_listing_functionality() -> Result<()> {
    ...
    assert!(found_tools >= 2, "Should find at least 2 expected system tools, found {}", found_tools);
    ...
    println!("🎯 Expected tools found: {}/{}", found_tools, expected_tools.len());
...
async fn test_repl_tool_execution_with_error_handling() -> Result<()> {
    ...
    let result = registry.execute_tool("system_info", &["extra", "args"]).await;
...
async fn test_repl_tool_output_validation() -> Result<()> {
                ...
                match serde_json::from_str::<serde_json::Value>(&result.output) {
                    Ok(json_value) => {
                        ...
                        if tool_name == "system_info" {
                            assert!(json_value.as_object().is_some(), "system_info should return object");
                        }
                    }
                    Err(_) => {
                        println!("⚠️  {} produces non-JSON output (might be expected)", tool_name);
                    }
                }
            }
            Err(e) => {
    ...
    assert!(successful_json > 0, "Should have at least one tool producing valid JSON");

    println!("✅ Tool output validation completed");
    println!("📊 Tested: {}, Valid JSON: {}", tested_tools, successful_json);
...
async fn test_repl_config_initialization() -> Result<()> {
    ...
    assert!(context.db_path.to_str().is_some(), "DB path should be valid");
