...
async fn test_tool_registry_initialization() -> Result<()> {
    ...
    let registry = timeout(Duration::from_secs(10), UnifiedToolRegistry::new(tool_dir.clone()))
        .await
    ...
    assert!(!tools.is_empty(), "Tool registry should have tools available");

    // Check for expected system tools
    let expected_tools = vec!["system_info", "list_files", "read_file"];
    let found_expected_tools: Vec<String> = expected_tools.iter()
...
async fn test_list_files_execution(registry: &UnifiedToolRegistry) -> Result<()> {
    ...
    let result = registry.execute_tool("list_files", &[test_dir.to_string_lossy().to_string()]).await?;
    ...
    assert!(has_file_content || !output.is_empty(),
...
async fn test_tools_grouping() -> Result<()> {
    ...
    assert!(has_system_group || has_rune_group,
...
async fn test_invalid_tool_handling() -> Result<()> {
    ...
    if let Some(first_tool) = tools.first() {
        let result = registry.execute_tool(first_tool, &["invalid_arg".to_string(), "another_invalid".to_string()]).await;
            ...
            Ok(tool_result) => {
                match tool_result.status {
                    ...
                    crucible_cli::commands::repl::tools::ToolStatus::Error(ref error) => {
                        println!("✓ Tool '{}' correctly rejected invalid arguments: {}", first_tool, error);
                    }
                }
            }
            Err(e) => {
                println!("ℹ Tool '{}' failed to execute with invalid args: {}", first_tool, e);
            }
        }
    }
...
async fn test_tool_execution_performance() -> Result<()> {
    ...
    if !tools.is_empty() {
        ...
        assert!(execution_time < Duration::from_secs(10),
               "Multiple tool executions should complete within 10 seconds, took {:?}", execution_time);
