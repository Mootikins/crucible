...
impl ExtendedMcpServer {
    ...
    /// Create a new extended MCP server with explicit configuration
    ///
    /// This constructor uses the provided `Config` to configure discovery paths
    /// for Rune tools and event handlers. It respects:
    /// - `[discovery.tools]` for Rune tool discovery paths
    /// - `[discovery.events]` for event handler discovery paths
    ///
    /// # Arguments
    ///
    /// * `kiln_path` - Path to the kiln directory
    ...
    /// let server = ExtendedMcpServer::with_config(
    ///     "/path/to/kiln".to_string(),
    ///     knowledge_repo,
    ///     embedding_provider,
    ///     ".",
    ///     Some(&config),
    ...
    pub async fn with_config(
        ...
        embedding_provider: Arc<dyn EmbeddingProvider>,
        just_dir: impl AsRef<Path>,
        config: Option<&crucible_config::Config>,
    ) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        let just_dir = just_dir.as_ref();
        let discovery_config = config.and_then(|c| c.discovery_config());

        // Use config for Rune discovery
        let rune_config = crate::config_bridge::create_rune_discovery_config(
            Some(just_dir),
            discovery_config,
        );

        // Use config for event handler
        let event_handler_config = crate::config_bridge::create_event_handler_config(
            Some(just_dir),
            discovery_config,
        );
        ...
        // Create Just tools wrapper
        let just_dir = just_dir.to_path_buf();
        let just_tools = Arc::new(JustTools::new(&just_dir));
        if just_tools.has_justfile() {
            if let Err(e) = just_tools.refresh().await {
                warn!("Failed to load justfile: {}", e);
        ...
        // Create Rune registry using config-based discovery
        let rune_registry = Arc::new(RuneToolRegistry::discover_from(rune_config).await?);
        let rune_count = rune_registry.tool_count().await;
        info!("Loaded {} Rune tools (from config)", rune_count);

        // Create event handler using config-based discovery
        let event_handler = match EventHandler::new(event_handler_config) {
            Ok(handler) => {
                // Ensure event directories exist
                if let Err(e) = handler.ensure_event_directories(&["recipe_discovered"]) {
                    warn!("Failed to ensure event directories: {}", e);
                }
                info!("Recipe event handler initialized (from config)");
                Some(Arc::new(handler))
            }
            Err(e) => {
                warn!("Failed to create event handler: {}", e);
