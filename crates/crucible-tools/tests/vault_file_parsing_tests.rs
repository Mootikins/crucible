...
use crucible_tools::vault_types::VaultResult;
use serde_json::Value;
use chrono::Datelike;

/// Test vault path for integration testing
const TEST_VAULT_PATH: &str = "~/Documents/crucible-testing";
...
async fn test_vault_scanner_finds_markdown_files() {
    ...
    assert!(!markdown_files.is_empty(), "Should find markdown files in test vault");

    // Verify all entries are markdown files
    for file_path in &markdown_files {
        assert!(file_path.extension().unwrap_or_default() == "md",
                "All files should have .md extension, found: {:?}", file_path);
    }

    // Verify we found expected files from the test vault
    let file_paths: Vec<String> = markdown_files.iter()
        .map(|p| p.to_string_lossy().to_string())
        .collect();

    // Should find known files in the test vault
    assert!(file_paths.iter().any(|p| p.contains("PRIME.md")),
            "Should find PRIME.md in test vault");
    assert!(file_paths.iter().any(|p| p.contains("Rune MCP")),
...
async fn test_frontmatter_parsing_extracts_metadata() {
    ...
    assert!(vault_file.metadata.frontmatter.contains_key("type"),
            "Should extract 'type' field from frontmatter");
    assert!(vault_file.metadata.frontmatter.contains_key("tags"),
    ...
    assert_eq!(frontmatter_type, &Value::String("meta".to_string()),
    ...
    if let Value::Array(tag_array) = tags {
        assert!(tag_array.contains(&Value::String("vault-config".to_string())),
                "Should contain 'vault-config' tag");
        assert!(tag_array.contains(&Value::String("instructions".to_string())),
...
async fn test_file_change_detection_via_hashing() {
    ...
    // Get initial hash
    let initial_hash = change_detector.calculate_file_hash(&test_file_path).await.unwrap();
    assert!(!initial_hash.is_empty(), "Hash should not be empty");
    assert_eq!(initial_hash.len(), 64, "SHA256 hash should be 64 characters");

    // Verify hash is consistent
    let second_hash = change_detector.calculate_file_hash(&test_file_path).await.unwrap();
    assert_eq!(initial_hash, second_hash, "Hash should be consistent for unchanged file");
    ...
    let different_hash = change_detector.calculate_file_hash(&different_file_path).await.unwrap();
    assert_ne!(initial_hash, different_hash, "Different files should have different hashes");
}
...
async fn test_vault_parsing_integration_with_real_files() {
    ...
    for vault_file in &parsed_files {
        assert!(!vault_file.path.to_string_lossy().is_empty(), "File path should not be empty");
        assert!(!vault_file.content.is_empty(), "Content should not be empty");
        assert!(!vault_file.hash.is_empty(), "Hash should not be empty");
        assert!(vault_file.metadata.size > 0, "File size should be greater than 0");
    ...
    let prime_file = parsed_files.iter()
        .find(|f| f.path.to_string_lossy().contains("PRIME.md"))
        .expect("Should find PRIME.md in parsed files");

    assert_eq!(prime_file.metadata.frontmatter.get("type").unwrap(),
               &Value::String("meta".to_string()));
    assert!(prime_file.content.contains("# Vault Primer"),
...
async fn test_error_handling_missing_file() {
    ...
    match result.unwrap_err() {
        crucible_tools::vault_types::VaultError::FileNotFound(path) => {
            assert!(path.contains("nonexistent_file.md"), "Error should mention missing file");
        }
...
async fn test_error_handling_malformed_frontmatter() {
    ...
    let _error_type = crucible_tools::vault_types::VaultError::FrontmatterParseError(
...
async fn test_vault_scanner_recursive_directory_traversal() {
    ...
    assert!(all_files.len() > root_only_files.len(),
    ...
    let has_subdir_files = all_files.iter().any(|p| {
        p.to_string_lossy().contains("Projects/") ||
        p.to_string_lossy().contains("Sessions/")
    });
    assert!(has_subdir_files, "Recursive scan should include subdirectory files");
}
...
async fn test_vault_metadata_extraction_various_frontmatter_types() {
    ...
    for file_name in test_files {
        ...
        if frontmatter.contains_key("created") || frontmatter.contains_key("date") {
            let date_field = frontmatter.get("created")
        ...
        assert!(vault_file.content.contains("#"),
...
// not real data from the test vault at ~/Documents/crucible-testing
...
async fn test_search_by_properties_returns_real_vault_data_not_mock_data() {
    ...
    assert!(!matching_files.is_empty(), "Should find files with type=meta");
...
async fn test_get_vault_stats_calculates_real_statistics_not_mock_numbers() {
    ...
    assert_ne!(total_notes, 1250,
              "Expected real vault note count, not mock value of 1250. Got: {}", total_notes);
    assert_ne!(total_size_mb, 156.7,
              "Expected real vault size calculation, not mock value of 156.7. Got: {}", total_size_mb);
    ...
    assert!(!is_perfect_round_number,
           "Real vault size should not be a perfect round number like mock data. Got: {}", total_size_mb);
}
...
async fn test_list_tags_extracts_real_tags_from_vault_frontmatter() {
    ...
    assert_ne!(total_tags, 3,
              "Expected real tag count from vault, not mock value of 3. Got: {}", total_tags);
}
...
async fn test_metadata_extraction_normalizes_tags_and_dates() {
    ...
    for tag in &tags {
        ...
        for c in tag.chars() {
            if c.is_alphabetic() {
                assert!(c.is_ascii_lowercase(), "Character '{}' in tag '{}' should be lowercase", c, tag);
            }
        }
        assert!(!tag.contains(' '), "Tag '{}' should use hyphens instead of spaces", tag);
    }

    // Test date parsing - PRIME.md has created date
    if let Some(created_date) = vault_file.metadata.created {
        assert!(created_date.year() >= 2020, "Created date should be realistic");
        assert!(created_date.year() <= 2025, "Created date should not be in future");
    }
}
...
async fn test_metadata_extraction_handles_various_frontmatter_formats() {
    ...
    for file_name in test_files {
        ...
        if std::path::Path::new(&full_path).exists() {
            ...
            assert!(!vault_file.metadata.frontmatter.is_empty(),
                    "{} should have frontmatter", file_name);
...
async fn test_metadata_extraction_calculates_content_metrics() {
    ...
    let has_headers = vault_file.content.lines().any(|line| line.trim().starts_with('#'));
...
async fn test_metadata_extraction_handles_relationships_and_links() {
    ...
    if std::path::Path::new(&test_file_path).exists() {
        ...
        assert!(!content_lines.is_empty(), "Should have content lines to analyze");
    }
}