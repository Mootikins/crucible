...
// =============================================================================
// Kiln tool tests (baseline 14 tools = 12 kiln + 2 discovery)
// =============================================================================
...
async fn test_kiln_tools_always_present() {
    ...
    assert_eq!(tools.len(), 14, "Should have 12 kiln + 2 discovery tools");
    ...
    // DiscoveryTools (2)
    assert!(tool_names.contains(&"discover_tools"));
...
"#,
    ...
    assert_eq!(
        tools.len(),
        16,
        "Should have 12 kiln + 2 discovery + 2 rune tools"
    );
...
"#,
    ...
    assert_eq!(tools.len(), 17); // 12 kiln + 2 discovery + 1 legacy + 2 modern
...
async fn test_multiple_rune_directories() {
    ...
    assert_eq!(tools.len(), 16); // 12 kiln + 2 discovery + 2 rune
...
async fn test_rune_refresh_discovers_new_tools() {
    ...
    assert_eq!(server.tool_count().await, 15); // 12 kiln + 2 discovery + 1 rune
    ...
    assert_eq!(server.tool_count().await, 16); // 12 kiln + 2 discovery + 2 rune
}
...
async fn test_nonexistent_rune_directory_handled() {
    ...
    assert_eq!(server.tool_count().await, 14); // 12 kiln + 2 discovery, 0 rune
}
...
async fn test_recursive_rune_discovery() {
    ...
    assert_eq!(
        tools.len(),
        17,
        "12 kiln + 2 discovery + 3 rune (recursive)"
    );
