...
use crate::config::includes::IncludeConfig;
use crate::config::{EnrichmentConfig, ProfileConfig};
        ...
        if let Some(gateway) = &self.gateway {
            ...
            for server in &gateway.servers {
                ...
                match &server.transport {
                    crate::config::components::gateway::TransportType::Stdio {
                        command, ..
                    } => {
                        if command.is_empty() {
                            errors.push(format!(
                    ...
                    crate::config::components::gateway::TransportType::Sse { url, .. } => {
                        if url.is_empty() {
                            errors.push(format!(
    ...
    pub fn validate_discovery(&self) -> Result<(), ConfigValidationError> {
        if let Some(discovery) = &self.discovery {
            ...
            let mut check_type_config =
                |type_name: &str, config: &crate::config::components::TypeDiscoveryConfig| {
...
/// `..` — callers should warn rather than create a directory with an
/// unsafe name.
