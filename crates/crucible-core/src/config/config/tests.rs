...
use super::*;
use crate::config::components::{ChatConfig, DiscoveryPathsConfig, GatewayConfig, HandlersConfig};
...
fn test_validate_gateway_empty_name() {
    let config = Config {
        gateway: Some(GatewayConfig {
            servers: vec![crate::config::components::gateway::UpstreamServerConfig {
                name: "".to_string(),
                transport: crate::config::components::gateway::TransportType::Stdio {
...
fn test_validate_gateway_invalid_sse_url() {
    let config = Config {
        gateway: Some(GatewayConfig {
            servers: vec![crate::config::components::gateway::UpstreamServerConfig {
                name: "test".to_string(),
                transport: crate::config::components::gateway::TransportType::Sse {
...
fn test_validate_handlers_empty_pattern() {
    let config = Config {
        handlers: Some(HandlersConfig {
            builtin: crate::config::components::BuiltinHandlersTomlConfig {
                test_filter: crate::config::components::HandlerConfig {
                    enabled: true,
...
fn test_validate_discovery_empty_path() {
    ...
    type_configs.insert(
        "tools".to_string(),
        crate::config::components::TypeDiscoveryConfig {
    ...
    let config = Config {
        discovery: Some(crate::config::components::DiscoveryPathsConfig {
            handlers: None,
            tools: None,
            events: None,
            type_configs,
...
fn test_effective_llm_provider_without_llm_default_returns_error() {
    let config = Config {
        ...
        chat: Some(ChatConfig {
            ...
            agent_preference: crate::config::components::AgentPreference::default(),
...
"#;
    ...
    assert_eq!(
        cloud.provider_type,
        crate::config::components::BackendType::OpenAI
    );
...
fn include_config_default_all_none() {
    let inc = crate::config::includes::IncludeConfig::default();
    assert!(
        inc.is_empty(),
...
gateway = "mcps.toml"
"#;
    let inc: crate::config::includes::IncludeConfig = toml::from_str(toml_content).unwrap();
