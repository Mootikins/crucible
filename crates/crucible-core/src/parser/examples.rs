...
use crate::parser::{
    StorageAwareMarkdownParser, ParserStorageCoordinator, ParsingOperation,
    OperationType, OperationPriority, OperationMetadata, CoordinatorConfig,
...
"#;
    ...
    let result = parser.parse_content_with_storage(content, source_path, Some(storage)).await?;
    ...
    println!("   💾 Storage time: {}ms", result.statistics.storage_time_ms);
    println!("   📦 Content size: {} bytes", result.statistics.content_size_bytes);
    println!("   🔄 Deduplication ratio: {:.2}", result.statistics.deduplication_ratio);
    println!("   🔀 Parallel processing: {}", result.statistics.parallel_processing_used);
...
pub async fn batch_processing_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let coordinator = coordinator_factory::create_default_coordinator(Some(coordinator_config)).await?;
    ...
    println!("   🔄 Average deduplication ratio: {:.3}", stats.average_deduplication_ratio);
    println!("   ⏱️  Average parse time: {:.1}ms", stats.average_parse_time_ms);
    println!("   💾 Average storage time: {:.1}ms", stats.average_storage_time_ms);
...
"#;
    ...
    let original_result = parser.parse_content_with_storage(
        original_content,
        source_path,
...
"#;
    ...
    let modified_result = parser.parse_and_compare(
        modified_content,
        source_path,
        &original_result,
        Some(Arc::clone(&storage))
        ...
        for (i, change) in changes.iter().enumerate() {
            match change {
                crate::storage::EnhancedTreeChange::AddedBlock { index, hash, .. } => {
                    println!("   ➕ {}: Added block at index {} (hash: {})", i + 1, index, &hash[..8]);
                }
                ...
                crate::storage::EnhancedTreeChange::DeletedBlock { index, hash, .. } => {
                    println!("   ➖ {}: Deleted block at index {} (hash: {})", i + 1, index, &hash[..8]);
                }
                crate::storage::EnhancedTreeChange::MovedBlock {
                    ...
                    println!("   ↔️  {}: Moved block {} → {} (hash: {})", i + 1, old_index, new_index, &hash[..8]);
                }
                _ => {
    ...
    println!("   📦 Content size: {:8} | {:8}", original_stats.content_size_bytes, modified_stats.content_size_bytes);
    println!("   🧱 Block count:   {:8} | {:8}", original_stats.block_count, modified_stats.block_count);
    println!("   ⏱️  Parse time:    {:8}ms | {:8}ms", original_stats.parse_time_ms, modified_stats.parse_time_ms);
...
pub async fn performance_optimization_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let parser_config = StorageAwareParserConfig {
        block_size: BlockSize::Adaptive { min: 1024, max: 16384 }, // Adaptive sizing
        enable_storage: true,
    ...
    println!("📄 Generated large document ({} lines, {} bytes)",
    ...
    println!("🌳 Merkle tree depth: {}", parse_result.merkle_tree.as_ref().map_or(0, |t| t.depth));
    ...
    println!("   🔄 Deduplication ratio: {:.3}", stats.deduplication_ratio);
    println!("   🚀 Parallel processing: {}", stats.parallel_processing_used);

    // Calculate processing throughput
    if stats.parse_time_ms > 0 {
        let throughput_mbps = (stats.content_size_bytes as f64) / (stats.parse_time_ms as f64) * 1000.0 / (1024.0 * 1024.0);
...
pub async fn error_handling_example() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, result) in batch_result.operation_results.iter().enumerate() {
        println!("   {} {}: {}",
                 ...
                 if result.success {
                     ...
                     result.error.as_ref().unwrap_or(&"Unknown error".to_string()).clone()
                 });
    }
    ...
    if !batch_result.success {
        ...
        if !failed_operations.is_empty() {
            ...
            println!("🔄 Retry completed: {} successful, {} failed",
                     retry_result.successful_operations,
                     retry_result.failed_operations);
        }
    }
...
pub async fn run_all_examples() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("This demo showcases the comprehensive integration between parsing and storage systems.\n");
