...
impl StubGenerator {
    ...
    pub fn write_declarations(lua: &Lua, path: &Path) -> Result<(), LuaError> {
        ...
        // A VM with NO `cru` is a real shape, not a broken one: a theme file
        // evaluates on a bare `Lua::new()` and calls nothing at all. Its
        // definitions file must say so, because the alternative is what this
        // fixes — a theme checked against a file that advertised `cru.hl`,
        // passing the gate and raising "attempt to index nil with 'hl'" the
        // moment it loaded.
        //
        // The static `DECLARED` table is skipped along with the walk. Those
        // entries describe API bound at load on a VM that HAS `cru`; on a bare
        // one they would be the same lie in a different place.
        if lua.globals().get::<Table>("cru").is_err() {
            fs::write(
                path,
                "--!strict\n\
                 -- Generated by `cru plugin stubs`. Do not edit.\n\
                 -- This VM has no `cru` table. A file checked against these\n\
                 -- declarations may use the Luau standard library and nothing\n\
                 -- the host adds.\n",
            )?;
            return Ok(());
        }
