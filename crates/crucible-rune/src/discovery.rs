...
impl ToolDiscovery {
    ...
    pub async fn discover_from_file<P: AsRef<Path>>(&self, file_path: P) -> Result<DiscoveredTools> {
        ...
        let file_hash = Some(format!("{:x}", md5::compute(source_code.as_bytes())));
    ...
    pub async fn validate_file<P: AsRef<Path>>(&self, file_path: P) -> Result<ValidationResult> {
        ...
        if vm.call(["call"], (serde_json::json!({}),)).is_err() {
            validation.valid = false;
    ...
    fn generate_function_schema(&self, function: &crate::AsyncFunctionInfo) -> Result<Value> {
    ...
    /// Infer output schema
    fn infer_output_schema(&self, function: &crate::AsyncFunctionInfo) -> Option<Value> {
