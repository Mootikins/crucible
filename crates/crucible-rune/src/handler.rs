...
use crate::types::ToolExecutionConfig;
use anyhow::Result;
...
impl DynamicRuneToolHandler {
    ...
    pub fn update_config(&mut self, config: ToolExecutionConfig) {
        self.config = config;
    }
...
impl ToolHandlerGenerator {
    ...
    pub fn with_config(config: ToolExecutionConfig) -> Self {
        Self {
    ...
    pub fn generate_handlers_with_configs(
        ...
        tools: Vec<Arc<RuneTool>>,
        context: Arc<rune::Context>,
        config_fn: impl Fn(&RuneTool) -> ToolExecutionConfig,
    ) -> std::collections::HashMap<String, DynamicRuneToolHandler> {
        let mut handlers = std::collections::HashMap::new();
