...
impl RuneTool {
    ...
    pub fn to_tool_definition(&self) -> ToolDefinition {
        ToolDefinition {
            ...
            metadata: {
                let mut meta = HashMap::new();
                meta.insert("source_type".to_string(), Value::String("rune".to_string()));
                meta.insert("created_at".to_string(), Value::String(self.created_at.to_rfc3339()));
                meta.insert("modified_at".to_string(), Value::String(self.modified_at.to_rfc3339()));
                if let Some(file_path) = &self.file_path {
                    meta.insert("file_path".to_string(), Value::String(file_path.to_string_lossy().to_string()));
                }
                meta
            },
        }
