...
pub trait DatabaseConnection: Send + Sync {
    ...
    async fn execute_query(&self, query: &str, params: Vec<DatabaseValue>) -> Result<QueryResult>;

    /// Execute a statement that doesn't return results
    async fn execute_statement(&self, statement: &str, params: Vec<DatabaseValue>) -> Result<StatementResult>;
...
impl DatabaseConnection for DuckDBConnection {
    async fn execute_query(&self, query: &str, params: Vec<DatabaseValue>) -> Result<QueryResult> {
        ...
        while let Some(row) = results.next()? {
            ...
            for i in 0..results.column_count() {
                let value = match row.get::<_, duckdb::Value>(i)? {
                    duckdb::Value::Null => DatabaseValue::Null,
                    duckdb::Value::Boolean(b) => DatabaseValue::Bool(b),
                    duckdb::Value::TinyInt(i) => DatabaseValue::Integer(i as i64),
                    duckdb::Value::SmallInt(i) => DatabaseValue::Integer(i as i64),
                    duckdb::Value::Int(i) => DatabaseValue::Integer(i),
                    duckdb::Value::BigInt(i) => DatabaseValue::Integer(i),
                    duckdb::Value::UTinyInt(i) => DatabaseValue::Integer(i as i64),
                    duckdb::Value::USmallInt(i) => DatabaseValue::Integer(i as i64),
                    duckdb::Value::UInt(i) => DatabaseValue::Integer(i as i64),
                    duckdb::Value::UBigInt(i) => DatabaseValue::Integer(i as i64),
                    duckdb::Value::Float(f) => DatabaseValue::Float(f),
                    duckdb::Value::Double(f) => DatabaseValue::Float(f),
                    duckdb::Value::Timestamp(t) => DatabaseValue::String(t.to_rfc3339()),
                    duckdb::Value::Text(s) => DatabaseValue::String(s),
                    duckdb::Value::Blob(b) => DatabaseValue::Binary(b),
                    duckdb::Value::Date32(d) => DatabaseValue::String(d.to_string()),
                    duckdb::Value::Time64(t) => DatabaseValue::String(t.to_string()),
                };
                row_data.push(value);
            }
            rows.push(row_data);
...
pub struct SurrealDBConnection {
    client: Arc<crucible_surrealdb::SurrealDB>,
    connection_info: ConnectionInfo,
}
...
impl SurrealDBConnection {
    ...
    pub async fn new(config: DatabaseConfig) -> Result<Self> {
        ...
        let client = crucible_surrealdb::SurrealDB::new(&config.connection_string)
            .await
            .map_err(|e| anyhow::anyhow!("Failed to connect to SurrealDB: {}", e))?;

        // Sign in and use namespace/database
        client.signin(rune::runtime::Object::new()).await?;
...
impl DatabaseConnection for SurrealDBConnection {
    async fn execute_query(&self, query: &str, _params: Vec<DatabaseValue>) -> Result<QueryResult> {
    ...
    async fn execute_statement(&self, _statement: &str, _params: Vec<DatabaseValue>) -> Result<StatementResult> {
...
fn convert_to_duckdb_value(value: DatabaseValue) -> duckdb::Value {
    match value {
        DatabaseValue::Null => duckdb::Value::Null,
        DatabaseValue::Bool(b) => duckdb::Value::Boolean(b),
        DatabaseValue::Integer(i) => duckdb::Value::BigInt(i),
        DatabaseValue::Float(f) => duckdb::Value::Double(f),
        DatabaseValue::String(s) => duckdb::Value::Text(s),
        DatabaseValue::Binary(b) => duckdb::Value::Blob(b),
        DatabaseValue::Array(_) => duckdb::Value::Text("array".to_string()), // Simplified
        DatabaseValue::Object(_) => duckdb::Value::Text("object".to_string()), // Simplified
    }
}
...
impl DatabaseManager {
    ...
    pub async fn remove_connection(&self, name: &str) -> Result<bool> {
        let mut connections = self.connections.write().await;
