...
use crucible_watch::{FileEvent, FileEventKind, Error, Result};

// Import event-driven embedding components
use crucible_watch::{
    EmbeddingEventMetadata, EventDrivenEmbeddingConfig, EmbeddingEventResult,
    create_embedding_metadata, generate_document_id,
};
...
impl MockEmbeddingDatabase {
    ...
    async fn get_embedding_by_document_id(&self, document_id: &str) -> Option<StoredEmbedding> {
        let embeddings = self.embeddings.read().await;
        embeddings.iter().find(|e| e.document_id == document_id).cloned()
    }
}
...
impl EmbeddingIntegrationTest {
    ...
    async fn process_single_file_event(&self, event: FileEvent) -> Result<EmbeddingEventResult> {
        ...
        let embedding_metadata = crucible_watch::create_embedding_metadata(
            &event.path,
            &event.kind,
            file_size,
        );
...
async fn test_file_event_triggers_automatic_embedding_generation() {
    ...
    let file_events = test_setup.setup_file_events(&file_paths).await
    ...
    for (i, event) in file_events.iter().enumerate() {
        assert_eq!(event.kind, FileEventKind::Created, "Event {} should be Created event", i);
    ...
    let embedding_results = test_setup.process_file_events(file_events).await
    ...
    assert_eq!(embedding_results.len(), 4, "Should have 4 embedding results");

    for (i, result) in embedding_results.iter().enumerate() {
        assert!(result.success, "Embedding result {} should succeed", i);
        assert!(result.embedding_dimensions.is_some(), "Result {} should have embedding dimensions", i);
        assert_eq!(result.embedding_dimensions.unwrap(), 384, "Embedding should be 384 dimensions");
        assert!(result.processing_time > Duration::from_millis(0), "Processing time should be positive");
        assert!(result.error.is_none(), "Result {} should not have errors", i);
    }
    ...
    let search_results = test_setup.verify_semantic_search().await
    ...
    for result in &search_results {
        ...
        assert!(!result.content_preview.is_empty(), "Content preview should not be empty");
    ...
    assert!(processing_duration < Duration::from_secs(5),
           "Total processing should be under 5 seconds, took: {:?}", processing_duration);

    // Average processing time per document should be reasonable
    let avg_time_per_doc = processing_duration / 4;
    assert!(avg_time_per_doc < Duration::from_millis(500),
           "Average processing time per document should be under 500ms, took: {:?}", avg_time_per_doc);

    // Phase 8: Integration validation
    println!("🔗 Validating end-to-end integration...");

    // Verify that we can find specific documents through semantic search
    let architecture_results = test_setup.database.semantic_search("event-driven architecture", 2).await
        .expect("Failed to search for architecture content");

    assert!(!architecture_results.is_empty(), "Should find architecture-related documents");
    ...
    let found_relevant_content = architecture_results.iter().any(|result| {
        ...
        result.content_preview.to_lowercase().contains("architecture")
    });
    assert!(found_relevant_content, "Search results should contain relevant content");
    ...
    let error_result = test_setup.process_single_file_event(non_existent_event).await;
    assert!(error_result.is_err(), "Processing non-existent file should return error");

    // Test with empty file (should still work)
    let empty_file_path = test_setup.vault.get_documents_path().join("empty.md");
    tokio::fs::write(&empty_file_path, "").await
        .expect("Failed to create empty test file");

    let empty_file_event = FileEvent::new(FileEventKind::Created, empty_file_path);
    let empty_result = test_setup.process_single_file_event(empty_file_event).await
        .expect("Failed to process empty file");

    assert!(empty_result.success, "Empty file should be processed successfully");
...
async fn test_event_batching_and_deduplication() {
    ...
    let mut events = Vec::new();
    for _ in 0..3 {
        events.push(FileEvent::new(FileEventKind::Modified, file_paths[0].clone()));
    }

    // Process events
    let results = test_setup.process_file_events(events).await
        .expect("Failed to process file events");

    // All events should be processed (deduplication logic would be in the real implementation)
    assert_eq!(results.len(), 3, "Should process all events (deduplication not implemented yet)");

    // But only one embedding should be stored (deduplication would prevent duplicates)
    let stored_count = test_setup.database.count_embeddings().await;
    assert_eq!(stored_count, 3, "Currently stores all embeddings (deduplication not implemented)");
...
async fn test_priority_based_embedding_processing() {
    ...
    // Create test content
    tokio::fs::write(&critical_file, "# Critical Document\nThis needs immediate processing.").await
        .expect("Failed to create critical document");
    tokio::fs::write(&normal_file, "# Normal Document\nStandard processing is fine.").await
        .expect("Failed to create normal document");
    tokio::fs::write(&low_file, "# Low Priority Document\nBackground processing only.").await
    ...
    let low_result = test_setup.process_single_file_event(low_event).await
        .expect("Failed to process low priority event");
    let normal_result = test_setup.process_single_file_event(normal_event).await
        .expect("Failed to process normal priority event");
    let critical_result = test_setup.process_single_file_event(critical_event).await
    ...
    assert!(normal_result.success, "Normal priority event should succeed");
    assert!(critical_result.success, "Critical priority event should succeed");
...
async fn test_graceful_shutdown_scenarios() {
    ...
    let first_half_events: Vec<FileEvent> = file_paths.iter().take(2).map(|path| {
        FileEvent::new(FileEventKind::Created, path.clone())
    }).collect();

    let _ = test_setup.process_file_events(first_half_events).await
    ...
    let second_half_events: Vec<FileEvent> = file_paths.iter().skip(2).map(|path| {
        FileEvent::new(FileEventKind::Created, path.clone())
    }).collect();

    let _ = test_setup.process_file_events(second_half_events).await
        .expect("Failed to process remaining events during shutdown");

    // Verify all embeddings are stored
    let stored_count = test_setup.database.count_embeddings().await;
    assert_eq!(stored_count, 4, "All embeddings should be stored despite shutdown");
