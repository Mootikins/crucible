...
use crucible_watch::{FileEvent, FileEventKind, Error, Result, EventHandler};

// Import event-driven embedding components
use crucible_watch::{
    EventDrivenEmbeddingConfig, EmbeddingEventResult,
    EventDrivenEmbeddingProcessor, EventProcessorMetrics,
};
...
async fn test_file_event_triggers_automatic_embedding_generation_missing_integration() {
    ...
    let test_vault = TestVault::new().await
        .expect("Failed to setup test vault");

    // Phase 2: Create test documents
    println!("📄 Creating test markdown documents...");
    let file_paths = test_vault.create_test_documents().await
        ...
        Ok(processor) => {
            ...
            match start_result {
                Err(e) => {
                    ...
                    assert!(format!("{:?}", e).contains("receiver not set") ||
                           format!("{:?}", e).contains("Embedding event receiver"),
                           "Error should indicate receiver not set, got: {:?}", e);
                }
    ...
    println!("🔗 Demonstrating the missing integration between crucible-watch and embedding system...");
    ...
    let config = EventDrivenEmbeddingConfig::default();
    let event_handler_processor = create_event_driven_processor(config).await
    ...
    assert!(event_handler.can_handle(&test_event), "Should handle markdown files");
...
async fn test_missing_file_event_to_embedding_integration() {
    ...
    match transformation_result {
        Ok(embedding_event) => {
            println!("✅ File event transformation works: document_id = {}", embedding_event.document_id);
...
async fn attempt_file_event_transformation(event: FileEvent) -> Result<crucible_watch::EmbeddingEvent> {
