...
use crucible_watch::{FileEvent, FileEventKind, EventHandler, Error, Result};

// Import embedding components
use crucible_surrealdb::{
    embedding_pool::{EmbeddingThreadPool, EmbeddingProviderIntegration},
...
use crucible_watch::{
    EmbeddingEvent, EmbeddingEventMetadata, EmbeddingEventPriority, EventDrivenEmbeddingConfig, EmbeddingEventResult,
    EventDrivenEmbeddingProcessor, EventProcessorMetrics, EmbeddingEventHandler,
};
...
"#;
        ...
        let embedding_event = transform_file_event_to_embedding_event(event).await.unwrap();
        ...
        assert_eq!(embedding_event.metadata.priority, EmbeddingEventPriority::Normal);
    ...
    async fn test_batch_event_processing_multiple_files() {
        ...
        for (i, file_path) in file_paths.iter().enumerate() {
            ...
            let result = process_file_event_with_batching(&processor, event).await.unwrap();
    ...
    async fn test_event_driven_batch_timeout_logic() {
        ...
        tokio::fs::write(&file_path, "# Timeout Test\nThis should trigger timeout.").await.unwrap();
    ...
    async fn test_error_handling_and_retry_scenarios() {
        ...
        tokio::fs::write(&file_path, problematic_content).await.unwrap();

        // Create an event that will fail
        let event = FileEvent::new(FileEventKind::Modified, file_path.clone());

        // Process the event with retry logic
        let result = process_file_event_with_retry(&processor, event).await.unwrap();
        ...
        tokio::fs::write(&normal_file_path, "# Normal Content\nThis should work fine.").await.unwrap();

        let normal_event = FileEvent::new(FileEventKind::Modified, normal_file_path);
        let normal_result = process_file_event_with_retry(&processor, normal_event).await.unwrap();
    ...
    async fn test_integration_with_embedding_pool_event_driven() {
        ...
        let embedding_pool = EmbeddingThreadPool::new_with_provider_config(
            pool_config,
        ...
        let processor = create_event_driven_processor_with_pool(event_config, Arc::new(embedding_pool)).await.unwrap();

        let temp_dir = TempDir::new().unwrap();

        // Create test files
        let test_files = vec![
            ("integration1.md", "# Integration Test 1\nTesting event-driven integration."),
            ("integration2.md", "# Integration Test 2\nMore content for integration testing."),
        ];
        ...
        for (filename, content) in test_files {
            ...
            let result = process_file_event_integration(&processor, event).await.unwrap();
            results.push(result);
        }
    ...
    async fn test_performance_improvements_over_polling() {
        ...
        for i in 0..num_files {
            ...
            let content = format!("# Performance Test {}\nContent for performance test number {}.", i, i);
            let file_path = temp_dir.path().join(filename);
            tokio::fs::write(&file_path, content).await.unwrap();
    ...
    async fn test_deduplication_of_identical_events() {
        ...
        tokio::fs::write(&file_path, "# Deduplication Test\nContent to test deduplication.").await.unwrap();

        // Send multiple identical events for the same file
        let mut results = Vec::new();
        for _ in 0..5 {
            let event = FileEvent::new(FileEventKind::Modified, file_path.clone());
            let result = process_file_event_with_deduplication(&processor, event).await.unwrap();
            results.push(result);
        }
    ...
    async fn test_priority_based_event_processing() {
        ...
        tokio::fs::write(&critical_file, "# Critical Document\nThis needs immediate processing.").await.unwrap();
        tokio::fs::write(&normal_file, "# Normal Document\nStandard processing is fine.").await.unwrap();
        tokio::fs::write(&low_file, "# Low Priority Document\nBackground processing only.").await.unwrap();

        // Create events with different priorities
        let critical_event = create_priority_event(&critical_file, FileEventKind::Modified, EmbeddingEventPriority::Critical);
        let normal_event = create_priority_event(&normal_file, FileEventKind::Modified, EmbeddingEventPriority::Normal);
        let low_event = create_priority_event(&low_file, FileEventKind::Modified, EmbeddingEventPriority::Low);
        ...
        let normal_result = process_priority_event(&processor, normal_event).await.unwrap();
        let critical_result = process_priority_event(&processor, critical_event).await.unwrap();
    ...
    async fn test_graceful_shutdown_of_event_processor() {
        ...
        tokio::fs::write(&file_path, "# Shutdown Test\nTesting graceful shutdown.").await.unwrap();
    ...
    async fn create_event_driven_processor(config: EventDrivenEmbeddingConfig) -> Result<Arc<EventDrivenEmbeddingProcessor>> {
        ...
        let provider_integration = crucible_surrealdb::embedding_pool::EmbeddingProviderIntegration::with_mock(
            256, // Mini model dimensions
            "test-mock-model".to_string(),
        );

        let embedding_pool = crucible_surrealdb::embedding_pool::EmbeddingThreadPool::new_with_provider_config(
            pool_config,
            provider_integration,
        ).await.map_err(|e| crucible_watch::Error::Other(e.to_string()))?;
    ...
    async fn process_file_event_after_shutdown(
        ...
        if processor.is_shutdown().await {
            return Err(crucible_watch::Error::Other("Processor is shutdown".to_string()));
        }
    ...
    async fn get_embedding_pool_metrics(processor: &EventDrivenEmbeddingProcessor) -> ThreadPoolMetrics {
    ...
    async fn get_processor_metrics(processor: &EventDrivenEmbeddingProcessor) -> EventProcessorMetrics {
