...
use crate::{config::WatchBackend, error::{Error, Result}};
use std::sync::Arc;
...
mod tests {
    ...
    #[tokio::test]
    async fn test_extended_registry_creation() {
        let registry = ExtendedBackendRegistry::new();
        let available = registry.inner().available_backends();
        assert!(!available.is_empty());
    }

    #[tokio::test]
    async fn test_optimal_backend_selection() {
        let registry = ExtendedBackendRegistry::new();

        let requirements = WatcherRequirements::high_performance();
        let backend = registry.select_optimal_backend(&requirements);
        assert!(backend.is_ok());

        let low_freq_requirements = WatcherRequirements::low_frequency();
        let backend = registry.select_optimal_backend(&low_freq_requirements);
        assert!(backend.is_ok());
    }

    #[test]
    fn test_requirements_creation() {
        let req = WatcherRequirements::high_performance();
        assert_eq!(req.use_case, WatcherUseCase::HighPerformance);
        assert_eq!(req.max_latency_ms, Some(50));

        let req = WatcherRequirements::low_frequency();
        assert_eq!(req.use_case, WatcherUseCase::LowFrequency);
        assert_eq!(req.max_latency_ms, Some(5000));

        let req = WatcherRequirements::editor_integration();
        assert_eq!(req.use_case, WatcherUseCase::EditorIntegration);
        assert_eq!(req.max_latency_ms, Some(1000));
    }

    #[test]
    fn test_requirements_customization() {
        let req = WatcherRequirements::default()
            .with_max_latency(200)
            .with_resource_priority(ResourcePriority::LowCpu);

        assert_eq!(req.max_latency_ms, Some(200));
        assert_eq!(req.resource_priority, ResourcePriority::LowCpu);
    }
    ...
    fn test_requirement_matching() {
        ...
        let requirements = WatcherRequirements::high_performance();
        assert!(registry.meets_requirements(&capabilities, &requirements));
