...
mod tests {
    ...
    impl EventHandler for MockHandler {
        async fn handle(&self, _event: FileEvent) -> Result<()> {
            if self.should_fail {
                Err(Error::Handler("Mock handler failure".to_string()))
            } else {
                Ok(())
            }
        }
    ...
    #[tokio::test]
    async fn test_composite_handler_sequential() {
        ...
        composite.add_handler(handler1).await;
        composite.add_handler(handler2).await;

        let event = FileEvent::new(FileEventKind::Created, PathBuf::from("test.md"));

        assert!(composite.can_handle(&event));
        assert_eq!(composite.handler_count().await, 2);

        let result = composite.handle(event).await;
        assert!(result.is_ok());
    }

    #[tokio::test]
    async fn test_handler_states() {
        let composite = CompositeHandler::new(CoordinationStrategy::Sequential);
    ...
    #[tokio::test]
    async fn test_priority_groups() {
        let composite = CompositeHandler::new(CoordinationStrategy::PriorityGroups);

        let high_priority = Arc::new(MockHandler {
            name: "high",
            priority: 200,
            should_handle: true,
            should_fail: false,
        ...
        let low_priority = Arc::new(MockHandler {
            name: "low",
            priority: 50,
            should_handle: true,
            should_fail: false,
        ...
        composite.add_handler(high_priority).await;
        composite.add_handler(low_priority).await;
        ...
        let states = composite.get_handler_states().await;
        assert_eq!(states.get("high").unwrap().success_count, 1);
        assert_eq!(states.get("low").unwrap().success_count, 1);
    }
}