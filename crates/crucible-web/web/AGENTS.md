...
- **Live tier** (`playwright.live.config.ts`): `e2e/live/global-setup.ts` boots `cru web` on an isolated `$CRUCIBLE_SOCKET` against a TempDir kiln and scrubs provider credentials so no real LLM call can happen. Provide the binary via `CRU_BIN=…/target/debug/cru`; otherwise it skips (setup self-cleans on any failure path). It is **intentionally NOT wired into GitHub CI** — a conscious `CRU_BIN` gate that avoids adding a `cru` build to the bun-only `test-web` job. To run it in CI, build `cru` first (`cargo build -p crucible-cli --bin cru`) and set `CRU_BIN`.
