...
describe("BorderNode > dockState attribute", () => {
  ...
  it("Action.setDockState normalizes hidden to collapsed (2-state)", () => {
    const model = Model.fromJson(dockFixture);
    const right = getBorder(model, "right");
    expect(right.getDockState()).toBe("expanded");

    model.doAction({ type: "SET_DOCK_STATE", data: { nodeId: right.getId(), state: "hidden" } } as any);
    expect(right.getDockState()).toBe("collapsed");
  });

  it("Action.setDockState 2-state cycle: expanded → collapsed → expanded", () => {
    const model = Model.fromJson(dockFixture);
    const bottom = getBorder(model, "bottom");
...
describe("BorderNode > JSON round-trip serialization", () => {
  ...
  it("round-trips after Action mutations", () => {
    ...
    expect(restoredTop.getDockState()).toBe("collapsed");
  ...
  it("round-trips preserves all borders and their dock attributes", () => {
    ...
    model.doAction(Action.setDockState(getBorder(model, "right").getId(), "collapsed"));
    ...
    expect(getBorder(restored, "right").getDockState()).toBe("collapsed");
