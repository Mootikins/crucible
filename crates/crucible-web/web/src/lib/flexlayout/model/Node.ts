...
import { IJsonBorderNode, IJsonRowNode, IJsonTabNode, IJsonTabSetNode } from "../types";

// Forward-declare Model type to avoid circular dependency
type Model = any;
...
export abstract class Node {
	...
	/** @internal */
	findDropTargetNode(windowId: string, dragNode: Node & IDraggable, _x: number, _y: number): DropInfo | undefined {
		let rtn: DropInfo | undefined;
		if (this.rect.contains(_x, _y)) {
			if (this.model.getMaximizedTabset(windowId) !== undefined) {
				rtn = this.model.getMaximizedTabset(windowId)!.canDrop(dragNode, _x, _y);
			} else {
				rtn = this.canDrop(dragNode, _x, _y);
				if (rtn === undefined) {
					if (this.children.length !== 0) {
						for (const child of this.children) {
							rtn = child.findDropTargetNode(windowId, dragNode, _x, _y);
							if (rtn !== undefined) {
								break;
							}
						}
