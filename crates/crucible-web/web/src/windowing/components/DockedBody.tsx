...
function EdgePanelResizeHandle(props: { position: EdgePanelPosition }) {
  ...
  return (
    <div
      ...
      class="relative flex-shrink-0 z-10 bg-control hover:bg-hover-wash active:bg-primary transition-colors after:content-[''] after:absolute w-px cursor-col-resize after:inset-y-0 after:-inset-x-1"
...
export const DockedBody: Component<{ position: EdgePanelPosition }> = (props) => {
  ...
  const innerStyle = () => {
    ...
    return {
      width: `${fullSize()}px`,
      translate: props.position === 'left' ? `${-off}% 0` : `${off}% 0`,
    };
  };
