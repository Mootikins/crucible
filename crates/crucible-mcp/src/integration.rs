...
pub async fn index_document(
    ...
    // Generate real embedding using the provider
    ...
    match db
        .store_embedding(file_path, &content, &embedding, &metadata)
        .await
    {
...
pub async fn search_documents(
    ...
    // Generate real embedding for query using the provider
    let embedding_result = provider
        .embed(query)
        .await
...
mod tests {
    ...
    /// Generate a test embedding for document content
    /// This is only used in unit tests and is NOT available in production code
    pub(crate) fn generate_test_embedding(content: &str) -> Vec<f32> {
    ...
    #[test]
    fn test_document_to_metadata_basic_fields() {
        let doc = DocumentNode::new("Test Document".to_string(), "Test content".to_string());

        let metadata = document_to_metadata(&doc);

        assert_eq!(metadata.file_path, "Test Document.md");
        assert_eq!(metadata.title, Some("Test Document".to_string()));
        assert_eq!(metadata.folder, "documents");
    }

    #[test]
    fn test_generate_test_embedding_deterministic() {
        let content1 = "Hello, world!";
        let content2 = "Hello, world!";
        let content3 = "Different content";

        let emb1 = generate_test_embedding(content1);
        let emb2 = generate_test_embedding(content2);
        let emb3 = generate_test_embedding(content3);
