// crates/crucible-mcp/tests/unit/mcp_integration/test_cross_model_query.rs
//
...
mod test_multi_model_helpers {
    include!("../../test_multi_model_helpers.rs");
}
...
async fn test_cross_model_query_error_handling() {
    ...
    // Test 5: Invalid parameter types
    let invalid_types_params = serde_json::json!({
        "query": {
            "relational": {
                "table": 123,  // Should be string
                "filter": true  // Should be string
            }
        }
    });
    ...
    // Should either fail to parse or return validation error
    if result.is_ok() {
        let result = result.unwrap();
        assert_tool_error(&result);
    } else {
...
/// Test that cross-model queries meet performance requirements
///
/// PERFORMANCE TARGET: < 200ms for cross-model query with small dataset
///
/// This test establishes a performance baseline for cross-model operations.
/// Note: This is aspirational for Phase 1B/1C; optimization may occur in REFACTOR phase.
#[tokio::test]
async fn test_cross_model_query_performance() {
    let ctx = TestContext::with_test_data().await.unwrap();
    ...
    let timer = PerformanceTimer::start("cross_model_query");
    ...
    // Performance assertion (may fail in initial implementation)
    // This is documentation of our performance goal
    if elapsed_ms > 200 {
        eprintln!(
            "WARNING: Cross-model query took {}ms, target is < 200ms",
            elapsed_ms
        );
        // Don't fail test in Phase 1B/1C, but log for optimization phase
    }

    // Verify result includes execution metadata
    #[derive(serde::Deserialize)]
    struct QueryResult {
        records: Vec<serde_json::Value>,
    ...
    // Tool should include execution time metadata
    assert!(
        data.execution_time_ms.is_some(),
        "Result should include execution time for monitoring"
    );
}
...
#[tokio::test]
async fn test_cross_model_query_empty_results() {
    let ctx = TestContext::with_test_data().await.unwrap();
    ...
    let params = serde_json::json!({
        "query": {
            "relational": {
                "table": "users",
                "filter": "age > 1000"  // No users this old
            }
        }
    });
    ...
    let data: QueryResult = parse_json_result(&result).unwrap();
    assert_eq!(data.records.len(), 0, "Should return empty array for no matches");
}

/// Test pagination parameters
#[tokio::test]
async fn test_cross_model_query_pagination() {
    let ctx = TestContext::with_test_data().await.unwrap();
    ...
    assert!(result.is_ok(), "Pagination should be supported");
    let result = result.unwrap();
    assert_tool_success(&result);
    ...
    assert!(data.records.len() <= 5, "Should respect limit parameter");

    // Should indicate if more results available
    // (has_more field in result)
}
