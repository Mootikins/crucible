...
use super::test_data::{TestFixtures, TestFileBuilder};
...
async fn test_e2e_search_and_update() {
    ...
    let files = vec![
        TestFileBuilder::new("project1.md").with_tags(vec!["project"]).build_file_info(),
    ];
    ...
    let results = client.search_by_tags(&[String::from("project")]).await.unwrap();
    assert_eq!(results.len(), 1);
    
    let mut props = HashMap::new();
    props.insert("reviewed".to_string(), serde_json::json!(true));
    let update_result = client.update_properties(&results[0].path, &props).await.unwrap();
