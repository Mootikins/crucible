...
async fn test_full_indexing_and_search_flow() {
    ...
    let index_result = server.handle_tool_call("index_vault", index_args).await.unwrap();
    assert!(index_result.success);
    
    let indexed_count = index_result.data.unwrap().get("indexed").unwrap().as_u64().unwrap();
    ...
    let content_result = server.handle_tool_call("search_by_content", content_search_args).await.unwrap();
    ...
    let semantic_result = server.handle_tool_call("semantic_search", semantic_args).await.unwrap();
    ...
    let tag_result = server.handle_tool_call("search_by_tags", tag_args).await.unwrap();
    ...
    let filename_result = server.handle_tool_call("search_by_filename", filename_args).await.unwrap();
...
async fn test_metadata_update_affects_search() {
    ...
    let files_result = server.handle_tool_call("search_by_filename", json!({"pattern": "file*.md"})).await.unwrap();
    ...
    let update_result = server.handle_tool_call("update_note_properties", update_args).await.unwrap();
    ...
    let search_result = server.handle_tool_call("search_by_properties", search_args).await.unwrap();
    ...
    let metadata_result = server.handle_tool_call("get_note_metadata", metadata_args).await.unwrap();
...
async fn test_multi_step_workflow() {
    ...
    let folder_result = server.handle_tool_call("search_by_folder", folder_args).await.unwrap();
    ...
    let processed_result = server.handle_tool_call("search_by_properties", processed_args).await.unwrap();
    ...
    let batch_result = server.handle_tool_call("search_by_properties", batch_args).await.unwrap();
    ...
    for file in &semantic_files[..2] { // Test first 2 files
...
async fn test_error_handling_workflow() {
    ...
    let nonexistent_result = server.handle_tool_call("get_note_metadata", json!({"path": "nonexistent.md"})).await.unwrap();
    assert!(!nonexistent_result.success);
    assert!(nonexistent_result.error.is_some());
    
    // 3. Update non-existent file
    let update_nonexistent_result = server.handle_tool_call("update_note_properties", json!({
        "path": "nonexistent.md",
    ...
    // 4. Unknown tool
    let unknown_tool_result = server.handle_tool_call("unknown_tool", json!({})).await.unwrap();
    ...
    let search_result = server.handle_tool_call("search_by_content", search_args).await.unwrap();
    assert!(search_result.success);
}
...
async fn test_performance_with_multiple_operations() {
    ...
    // Test folder search
    let folder_result = server.handle_tool_call("search_by_folder", json!({
        "path": "vault",
        "recursive": true
    })).await.unwrap();
    assert!(folder_result.success);
    ...
    let properties_result = server.handle_tool_call("search_by_properties", json!({
        "properties": {"test": "value"}
    })).await.unwrap();
