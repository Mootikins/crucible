...
A single client can connect to multiple servers simultaneously. Crucible's [[Help/Extending/MCP Gateway]] does exactly this, aggregating tools from many upstream servers into a unified namespace.
...
See [[Help/Extending/MCP Gateway]] for full configuration details and [[Help/Config/mcp]] for the configuration reference.
...
In Crucible's architecture, these protocols compose: ACP manages the agent process and conversation on the outside, while MCP provides the tools the agent calls during that conversation on the inside. See [[Help/Concepts/Agent Client Protocol]] for the ACP specification reference.
...
- [[Help/Concepts/Agent Client Protocol]]: ACP spec (uses MCP for tools)
