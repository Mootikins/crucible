...
| Command | Description |
|---------|-------------|
| `cru daemon` | Manage the Crucible daemon (start, stop, restart, status, logs; `serve` runs it in the foreground). It also stops itself: on SIGTERM or SIGINT, and after `server.idle_shutdown_minutes` (default 30) with no work outstanding — no connected client, no turn in flight, no background job — see [[Help/Configuration]] |
