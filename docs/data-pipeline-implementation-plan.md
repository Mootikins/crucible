...
### Agent 1: Implement Real Kiln File Parsing
**Objective**: Replace mock file discovery with real kiln directory scanning

**Tasks:**
- Scan actual kiln directory (configured kiln path) for .md files
...
**Files to Modify:**
- Replace mock implementations in `crates/crucible-tools/src/kiln_tools.rs`
...
**Objective**: Extract real metadata from actual kiln files

**Tasks:**
- Extract real tags, titles, folders from actual kiln files
...
**Files to Modify:**
- `crates/crucible-tools/src/kiln_tools.rs` - metadata extraction functions
...
- Ensure embeddings stay in sync with kiln state
...
- Test with your real kiln data and verify different queries work
...
- Test REPL queries return actual kiln documents, not mock data
...
**Test Scenarios:**
- Test: Modify file in kiln → Check embedding is updated → Verify search finds new content
...
- Test: Query different terms → Get different, relevant results from your actual kiln
...
4. **Interface**: CLI and REPL use real search functions with actual kiln data

## Success Criteria

- No mock data remains in production execution pipeline
- All tools work with configured kiln path (your kiln or test kiln)
- File changes correctly trigger re-processing and embedding updates
- User queries return results from actual kiln content
- Integration tests use test kiln for predictable data