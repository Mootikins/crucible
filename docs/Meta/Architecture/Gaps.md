...
| G54 | knowledge | `hash_blocks` always produces `Block` rows (4.1) | `ASTBlock` only when `BlockProcessingConfig.enabled`, default false (`parser/types/ast.rs:83`) | not-built | - |
...
| G57 | knowledge | `Embedding` is a separate entity with a `block` field (3.7) | `Enricher` computes block embeddings (`enrichment/service.rs:21`); search returns one vector per note (F28) | not-built | - |
