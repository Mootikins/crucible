...
(the clean room lacked an input, usually a security invariant that only the
containment work had established, or a live surface the product docs do not
name;
...
| G75 | knowledge | ~~`Proposal` is owned by `ProposalStore` inside Knowledge (3.25)~~ | RESOLVED 2026-09-14: the proposal is retired. A pass writes its notes with the note tools in its own session, so each write is a review hunk; no `Proposal` type, no staging directory, no `cru proposals` | resolved | — |
...
allow lists have different override semantics and documented the order in a
working note outside this kiln instead of a merge. One defect stays open from that
