...
- Owns: `SnapshotId` (one string: a bare hex id is a git tree, a `plain:` prefixed one names the plain store, so every journal already on disk still replays) and `RootBackend { Git, Plain }`. `RootBackend::of(&SnapshotId)` answers the backend, so a root carries no extra field; `detect` at open is the only place that chooses, and a rebase the only one that chooses again. Every seam — capture, changed paths, blob, snapshot exists, keep, ignored — matches the enum exhaustively, under a module deny of `wildcard_enum_match_arm`. A git call handed a plain id answers `WrongBackend`, never a panic. `Plain` snapshots a root the daemon cannot ask git about — a kiln outside a repository is the expected shape — as a manifest of one content hash per file, blobs stored once by hash under the daemon data root, with a `(size, mtime, inode)` stat key to skip a file that did not change.
