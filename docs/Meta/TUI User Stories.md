...
**Tests:** T1 item/element wire round-trips, region parsing and `rows_below_input` in `statusline_items.rs`/`statusline_lua.rs`, registry caps + dirty check + sanitising in `statusline_exprs.rs`; T2 evaluation, combinators, gutter and narrow-width badge survival in `components/status_items.rs` plus the statusline snapshots in `component_isolation_tests.rs`, and end-to-end region placement in `tests/region_placement_tests.rs` — RED-verify that one by rendering only one hardcoded row, which is the shape the bug had; T3 `FileChanged` dispatch in `server/file_event_hooks.rs`.
