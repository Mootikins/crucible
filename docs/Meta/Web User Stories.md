...
**Acceptance:** `changes` registers in the RIGHT region beside Activity and Backlinks. The queue is the *composed* diff (`session_base` → worktree) from `review.list_hunks`, not a list of tool calls, so the action unit is the composed hunk. The badge counts what is owed and the unreviewed filter drains to "nothing left" rather than to an empty panel. Reject is `set_state('rejected')` — one call, and the daemon reverts on disk in it; there is no second verb and no deferred mark. A hunk in flight refuses a second click. `external` hunks (the user's own edits, §5) render for context with NO reject affordance. Clicking a file opens it in the center buffer AND asks that buffer to scroll to the first hunk; expanding a hunk mounts a CodeMirror merge view (`HunkMergeView`) whose Accept and Reject controls call the daemon, never CodeMirror's own chunk action. Comments anchor to a **range**, not a hunk id (§6), and open comments list and resolve.
**Tests:** W1 (`ChangesPanel.test.tsx` — grouping/counts, external-no-reject, accept/reject payloads, in-flight guard, filter drain, empty and failed states, open-and-scroll, comment range, resolve; `lib/__tests__/review-store.test.ts` — optimistic accept + re-list, shared subscription, `review_changed` debounce, event-driven refresh; `lib/__tests__/review-api.test.ts` — the five REST calls and their error surfacing). W2 GAP: the accept-a-hunk-and-watch-it-leave journey — blocked on a deterministic fixture that produces a real composed diff; promote once the live tier can script a writing turn.
