...
### WS-220: Sessions live in the right pane; the center is for editing
**As a user**, starting or opening a session never takes over my editing surface: chat opens as a tab in a right-side split pane (created on demand), Obsidian-with-copilot shaped. The Edit/Session header mode pills are gone — there is no center "mode" to toggle (goEdit/goSession remain as command-palette commands).
**Acceptance:** `openSessionInChat` reuses the rightmost pane when the layout root is a horizontal split, else splits the rightmost pane (`splitPane(..., 'horizontal')` — originals stay left, chat lands in the new right pane and focuses it); existing session tabs focus in place with no layout churn.
**Tests:** W1 (`session-placement.test.ts` — split-on-first, reuse-on-second, focus-no-churn, stable `sessionPane`), W2 (`new-session-chat-tab.spec.ts` right-pane assertions, `windowing-regression` pills-gone, `windowing-comprehensive` reworked for the pre-existing session split). Live-verified (real browser: session tab opened in its own right pane).
