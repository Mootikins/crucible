...
**Acceptance:** a save is a whole write through `PUT /api/kiln/file`, and it carries the base the buffer was read from. A clean save clears dirty and moves the buffer's base to the hash the daemon answered. A stale save is refused: nothing is written, the buffer stays dirty, and the toast offers one action, **Save as conflict copy**, which writes the refused text beside the note under a dated name. A save the daemon never answered is queued in the outbox and the buffer goes clean; when that write lands from the outbox while the note is open, the buffer takes the answered hash, so its next save is not refused for the user's own write. A failure the daemon answered with (a 403 on a read-only project) keeps dirty + shows a toast with a retry. Content round-trips exactly (frontmatter, unicode, wikilinks untouched). Autosave (decision log 2026-09-11) arms only for a file inside a kiln; a stored `autosaveSeconds: 0` from before settings version 2 migrates once to 2, and a 0 chosen after it is kept.
**Tests:** W1 the write door (`lib/offline/__tests__/sync.test.ts` — `sends the base it was edited from when the daemon answers`, `refuses a stale save online and queues nothing`, `queues the writing when the daemon never answered`, `raises a refusal the daemon answered with, and queues nothing`), the buffer (`contexts/__tests__/EditorContext.test.tsx` — `keeps the buffer dirty and offers a conflict copy when the save is stale`, `the conflict copy action writes the unsaved text beside the note`, `marks the buffer clean and takes the answered hash on a clean save`, `a queued save marks the buffer clean and takes no base`, `a drained write moves the open buffer's base when the bases match`); W1 autosave (`FileViewerPanel.test.tsx` — a note autosaves after the interval, a project file never does, 0 stays off) and the migration (`lib/settings.test.ts`); W2 full round-trip via the real editor harness (`editor-roundtrip.story.spec.ts` — open → dirty ● → PUT body → clean; save-failure keeps dirty), **W2 through the SHIPPED app** (`editor-shipped-app.story.spec.ts` — real `App` mount, file opened via the product `openFileInEditor` path, real `FileViewerPanel`, Save button + Cmd-S), W4 save-lands-on-disk (`kiln-truth.live.spec.ts` — byte-exact temp-kiln file).
...
### WS-220: Sessions dock in the right edge panel; the center is for editing
**As a user**, starting or opening a session never takes over my editing surface: chat docks as a tab in the RIGHT EDGE PANEL (the collapsible sidebar), which auto-expands — Obsidian-with-copilot shaped. The Edit/Session header mode pills are gone — there is no center "mode" to toggle (goEdit/goSession remain as command-palette commands). Legacy session-less generic `chat` tabs are pruned from persisted layouts on restore (they made the active session render wherever they were docked).
**Acceptance:** `openSessionInChat` adds the session tab to `edgePanels.right`'s group, expands the panel, and activates the tab; further sessions stack in the same group; existing session tabs focus in place (wherever the user moved them) with no layout churn; center-tiling fallback only when the layout has no right group.
**Tests:** W1 (`session-placement.test.ts` — dock+expand, stacking, focus-in-place incl. user-moved tabs, fallback; `layout-serializer.test.ts` legacy-chat-tab prune), W2 (`new-session-chat-tab.spec.ts` edge-panel assertions, `windowing-regression` pills-gone). Live-verified (real browser: session tab docked in the expanded right sidebar).
