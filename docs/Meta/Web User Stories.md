...
**Extension (hover cards, 2026-07-16):** any surface can join the drag system by carrying a Tab payload — `DragSource` gained a group-less `'newTab'` variant placed by `placeNewTab` (pane center/split, tab bar, edge panel with expand-on-drop, floating window; duplicate identities focus instead of duplicating). The wikilink hover card's header has a drag grip carrying its note as a file tab: hover a link, drag the card into a side panel, the note opens there. W1: `tab-placement.test.ts`, `WikilinkHoverPreview.test.tsx` (newTab registration; grip absent without a DnD provider).
