...
**Tests:** W1 (`lib/offline/__tests__/*` — the store, kept kilns, the mirror and its modes, the outbox's four data-loss rules, the sync fallbacks, image hydration; `components/settings/__tests__/OfflineSettings.test.tsx`; `components/mobile/__tests__/OfflineBadge.test.tsx`). W4 (`kiln-truth.live.spec.ts` — an anchored edit's bytes on real disk, a stale anchor refused whole, and the note index carrying frontmatter but never the daemon's scope stamp). W2 GAP: no browser story drives a real offline→reconnect→drain cycle; it needs a served tier that can cut the network.
