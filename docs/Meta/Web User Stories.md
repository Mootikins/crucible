...
**Extension (hover cards as Hover Editors, 2026-07-16):** any surface can join the drag system by carrying a Tab payload — `DragSource` gained a group-less `'newTab'` variant placed by `placeNewTab` (pane split, tab bar, edge panel with expand-on-drop, floating window; duplicate identities focus instead of duplicating). The wikilink hover card's whole header is a drag-handle bar (Obsidian Hover Editor pattern): release on a tab bar / edge panel / split zone to dock, release anywhere else — including a pane body — to TEAR OFF into a floating editor window at the drop point (`resolveNewTabTarget`), which is movable, resizable, minimizable, and stacks with other floats. W1: `tab-placement.test.ts` (placement + drop policy + spawn-at-point clamping), `WikilinkHoverPreview.test.tsx` (newTab registration; handle absent without a DnD provider).
