...
**Acceptance:** Settings → Offline lists every kiln with "Don't keep", "Notes only" and "Everything, with images", the size it costs (notes and attachments apart), and the count of unsent edits with Send now; choosing a mode fetches the kiln WHOLE and turning it off drops exactly what it kept. A note opens from the network when the daemon answers and from the kept copy when it does not. A save the daemon never answered is queued and the app bar shows the count; a save the daemon ANSWERED and refused (403 on a read-only project) is raised, never queued. A queued write keeps the hash it was edited from, and a note the daemon changed meanwhile becomes a conflict copy beside it. The outbox refuses to drain into a daemon it did not come from. A cached attachment is drawn only through `<img>`.
**Tests:** W1 (`lib/offline/__tests__/*` — the memory store AND the shipped `idbStore` against `fake-indexeddb`, kept kilns, the mirror and its modes, the outbox's data-loss rules including a save that lands mid-drain, `networkSink.write`'s four read outcomes, the read that prefers queued writing, the offline→reconnect round trip, image hydration; `components/settings/__tests__/OfflineSettings.test.tsx`; `components/mobile/__tests__/OfflineBadge.test.tsx`).
**W4 GAP:** the live legs in `kiln-truth.live.spec.ts` prove Track B's anchored edits, NOT this story — nothing in WS-318's acceptance mentions an anchored edit, and Track B has no story of its own. **W2 GAP:** no browser story drives a real offline→reconnect→drain cycle; it needs a served tier that can cut the network. Both defects that made this story fail in practice — a kiln-relative path never joined, and an identity fetched at the one moment it cannot be — were found by review, not by a test, and a live drain would have caught either.
