...
- [x] **Live-preview markdown editing** — SHIPPED 2026-07-16 (WS-212): markdown defaults to Obsidian-style live preview — styled prose with syntax marks hidden, ONLY the construct under the cursor revealed raw, mono reserved for code (`live-preview.ts` over the lezer tree, GFM base); source mode is a corner toggle, reading view stays on Mod-Shift-E (WS-210). Remaining polish: callout/LaTeX renderers, checkbox/table widgets, transclusion embeds.
...
- [x] **File-tree explorer** — SHIPPED 2026-07-18 (WS-217, Phase 1): the flat notes list is now a real hierarchical tree (folders reconstructed from kiln-relative note paths; projects lazy-walk one folder level), with a top-right **root dropdown** switching between all registered **projects** and **kilns** as browse roots. Live file-change updates via a new `GET /api/fs/events` SSE channel (kilns patch in-memory, non-`.md` leaves ignored; projects refresh-on-interaction). Full keyboard/ARIA tree (`role=tree`, roving focus, Enter=open, F2 reserved), sort (name/modified), collapse-all, reveal-active, read-only right-click menu (Open/Copy path/Reveal). Projects list via a new daemon `fs.list_dir` RPC — registry-allowlisted (fail-closed), per-entry symlink containment, dotfiles + gitignored hidden by default (`show_ignored` reveals). Adopts `@ark-ui/solid` TreeView. **Phase-2 DnD SHIPPED 2026-07-18** (WS-218): drag any tree row to a folder/tree-root to MOVE on disk (daemon `fs.move`, same fail-closed threat model), to an editor pane to OPEN it there, or into editor text to INSERT a `[[wikilink]]`/rel-path — native-HTML5 drags via `@atlaskit/pragmatic-drag-and-drop` with an innermost-zone protocol. **Rename / mkdir / delete-to-trash SHIPPED 2026-07-19** (WS-219 right-click menus: inline ark rename → link-safe fs.move, fs.mkdir, fs.trash → `.crucible/trash/`); remaining Phase-2: between-drop-lines. **Phase 3 SHIPPED 2026-07-18**: deterministic resolved-link index (`note_links` v2) + wikilink rewrite on rename/move (`note.rename`/`note.move` RPCs; kiln `.md` `fs.move` delegates); ambiguous links warn-and-skip; backlinks read the same index. Seams kept: node `status` field for future git/diff decoration, daemon-as-single-write-authority.
