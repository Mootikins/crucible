...
- [x] **Reflection Pass (second avenue)** `P2` — Ships as the default `reflection` runtime plugin: on `on_session_end` a forked cheap-model subagent reviews the *finished* transcript and **proposes** kiln notes, staged in `KILN/.crucible/proposals/` (outside the index, so unreviewed proposals never reach [[Help/Concepts/Precognition|Precognition]] or search). `cru proposals {list,show,accept,reject}` is the human disposition surface — accept promotes a note into the kiln, reject deletes it. Propose-only by design, with Hermes-style DO-NOT-capture guardrails against kiln pollution — the explicit lesson from the removed `session-digest` (which auto-merged via LLM-judged dedupe). See [[Help/Concepts/Reflection Pass]]. Informed by Hermes Agent's background-review + curator loop (2026-06-28) · `crucible-daemon`, `crucible-lua`, `runtime/plugins/reflection`
...
- [x] **Execution Backends as Plugins** `P1` — Workspace tools (`bash`, `read_file`, `edit_file`, `write_file`) are intercepted via `pre_tool_call` hooks and routed to alternate backends; the agent core stays backend-agnostic. Reference: the `oci` runtime plugin (v0.3.0) runs workspace tools inside OCI containers via `podman`/`docker`/`nerdctl exec`, returning `{ handled = true, result = ... }`; the runtime is auto-detected and isolation that cannot be established fails closed rather than falling back to the host ([[Help/Extending/Container Isolation]]). Sandbox/container isolation is a **plugin, not a core concern** — the daemon never grows a per-backend abstraction · `runtime/plugins/oci/` · `crucible-lua`, `crucible-daemon`
