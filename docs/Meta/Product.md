...
- [-] **`cru search` Text Mode & FTS5 Index** `P0` — index-backed full-text search from the CLI · [[Help/CLI/search]] · `crucible-daemon` (storage)
  - **Gets you:** nothing that looks at note content. `cru search --type text` matches only `display_title`, `name` and `path`; the snippet shown beside a hit is read from disk *after* matching, which makes it look like content search.
  - **Proof:** _none — `crates/crucible-cli/src/commands/search.rs`:70-102 never reads bodies, and the FTS5 virtual table with BM25 at `storage/sqlite/fts.rs` has exactly one reference outside its own file (a `pub use`): nothing indexes into it and nothing queries it._
...
- [-] **Precognition Toggle** `P0` — `:set precognition` to turn injection off for a session · `crucible-cli`
  - **Gets you:** nothing. `:set precognition=off` changes a string in the `:set` readout; the agent still injects, and there is no way to turn it off from a running TUI session.
  - **Proof:** _none — `commands/set.rs:293` classifies the key as `SetEffect::TuiLocal` with no RPC action, and `AgentHandle` has no boolean `set_precognition` method. A `session.set_precognition` RPC exists and the web UI calls it; the TUI never does._
...
- [-] **Token Budget Tracking** `P0` — `context_budget` on `SessionAgent`, settable via RPC; `estimate_tokens` chars/4 heuristic · `crucible-daemon`, `crucible-core`
  - **Gets you:** a number you can set and read back. It feeds only the auto-compact predicate and the `cru.context.usage` percentage — and because it is unset by default, `usage.budget` and `usage.percent` are `0` for every real session.
  - **Proof:** _none beyond a set/get round-trip (`tests/rpc_config_agent_e2e.rs:585`) — the value never reaches the agent handle; see **Context Strategies**._
...
- [-] **Context Strategies** `P1` — `ContextStrategy::{Truncate, SlidingWindow, Summarize}` · `crucible-core`, `crucible-daemon`
  - **Gets you:** nothing. All three strategies are dead in production; no strategy has ever run against a real session. `:set context_strategy=summarize` is "accepted" and that is all it is.
  - **Proof:** _none — `GenaiAgentHandle.context_budget` is never set from `SessionAgent.context_budget` (`agent_factory.rs:643-651` chains only `with_deferrable_tools`/`with_plugin_tools`/`with_modes`), so `enforce_context_budget` returns `NoChange` at `genai_handle.rs:550-552` on every request. Separately, the LLM recap is untested at every level: every Summarize test asserts the static `[summary placeholder]`, and `summarize_via_backend` has zero test references._
...
- [x] **Progressive Tool Disclosure** `P2` — automatic deferral when mode-filtered tool schemas exceed 15% of the effective context budget · `crucible-daemon` (tools)
  ...
  - **Proof:** `crates/crucible-daemon/src/provider/genai_handle.rs`::visible_tools_over_budget_defers_gateway_and_adds_bridge, `::visible_tools_under_budget_attaches_all_and_no_bridge`, `::visible_tools_plan_mode_filters_writes_and_defers`; bridge unwrapping in `agent_manager/messaging/tool_call.rs mod invoke_tool_tests`. Caveat: the 15% share is computed against `context_budget`, which is never set in production, so the decision always uses `DEFAULT_ASSUMED_CONTEXT` — fixing **Context Strategies** changes this behaviour too.
...
- [-] **Bundled Help Skills** `P1` — help skills shipped at `runtime/crucible-help/skills` · `crucible-daemon` (skills)
  - **Gets you:** nothing from an installed binary. They are found from a dev `target/debug/cru` and silently never load for `~/.local/bin/cru`.
  - **Proof:** _none for the installed case — `skills/discovery.rs:282-325` tries only `$CRUCIBLE_RUNTIME` and `<exe_dir>/../../runtime`, omitting the installed `<prefix>/share/crucible/runtime/` layout that `runtime_defaults.rs:58-74` and `daemon_plugins/mod.rs:1064` both try first. Skills discovery is the one resolver that breaks the "one answer for where runtime files live" invariant._
...
- [-] **File Attachment** `P0` — `@file` context attachment in chat · `crucible-cli`
  - **Gets you:** a workspace-file completion popup on `@`, and the accepted completion inserts the literal path text. No file content is attached to anything — the agent receives the string `@src/main.rs` and can only get the content by choosing to call `read_file`.
  - **Proof:** _none — the accepted completion pushes onto `OilChatApp::attached_context`, which is write-only: declared at `chat_app/mod.rs:119`, written at `autocomplete.rs:599`, and read nowhere in the repo. There is no `@`-expansion on the send path and none in the daemon. The popup itself works and is well covered; the attachment does not exist._
- [-] **Rules Files** `P0` — project-level AI instructions loaded into the system prompt · [[Help/Rules Files]] · `crucible-core` (config)
  - **Gets you:** nothing. The config key parses and is never read.
  - **Proof:** _none — `ContextConfig::rules_files` has zero consumers outside its own module's tests. The loader was `crucible-context/src/layered_prompt.rs`, deleted wholesale on 2026-02-23; `agent_factory.rs:531` still carries the comment "Load rules files (already in system_prompt)" and nothing puts them there. The bullet's `.crucible/rules` path never matched the implementation either (defaults were `AGENTS.md`, `.rules`, `.github/copilot-instructions.md`), and `docs/Help/Rules Files.md` documents hierarchical loading that does not exist._
...
- [-] **Scripted Agent Control (daemon plugin VM)** `P0` — the same `cru.get_session()` surface from a daemon-side plugin · `crucible-lua`, `crucible-daemon`
  - **Gets you:** nothing. Every setter silently succeeds and changes nothing; every getter returns a default. `s.mode` always reads `"chat"` — a mode id that is not even in the registry.
  - **Proof:** _none — `NoopSessionRpc` is `impl SessionConfigRpc for NoopSessionRpc {}`, i.e. all no-op trait defaults, and it is what gets bound at every daemon site (plugin `session_start`, plugin `session_end`, `lua.init_session`). There is no daemon-side implementation of `SessionConfigRpc` at all. The separate and working surface is the hook parameter: `cru.on_session_start(function(session) … end)` does reach `SessionAgent` — two different Lua objects both spelled `session`._
- [-] **Lua `temperature` / `max_tokens`** `P0` — per-session sampling knobs from Lua · `crucible-lua`, `crucible-daemon`
  - **Gets you:** a value on `SessionAgent`. No LLM request ever carries it.
  - **Proof:** _none — `GenaiAgentHandle` has no temperature or max_tokens field, and the `ChatOptions` it builds sets only capture flags and `reasoning_effort`. The 2026-06-28 decision removed the *user-facing* knob for exactly this reason; the Lua surface, `SessionAgent`, agent-card frontmatter and RPC validation all still advertise both. Five layers of nothing._
...
- [-] **`cru.tools.call(name, args)`** `P1` — programmatic tool calling from Lua · `crucible-lua`, `crucible-daemon` (tools)
  - **Gets you:** a workspace tool really executes and returns its output to Lua — with **no permission check whatsoever**. Any loaded plugin can run `bash` with no prompt, in any mode, including plan mode.
  - **Proof:** _none for the "respects session permission scope" claim — `DaemonToolsBridge::call_tool` constructs `ExecutionContext::default()` (no session id, no gate) and calls `WorkspaceTools::execute_tool` directly; `ExecutionContext` carries no permission field at all. All enforcement lives on the agent dispatch path this bypasses. Execution itself is proven (`tests/acp_integration_e2e.rs::test_tools_bridge_call_tool_routes_correctly`); the gate is the missing half. No plugin in the repo calls it, so nothing depends on the current behaviour._
...
- [-] **Terminal Palette Colours** `P1` — `term4` / bare index / `bright_*` address the user's own terminal colours · `crucible-oil`, `crucible-cli`
  - **Gets you:** `term4` and bare indices work. **Every named colour and every `bright_*` name renders as the wrong palette slot** — normal names emit the bright slot and `bright_*` names emit the normal slot. Set `blue` in your colorscheme and configure your terminal's slot 4, and you get slot 12.
  - **Proof:** _none — and there is committed counter-evidence: the statusline snapshot shows `mode_normal: Green` (slot 2) and `primary: Cyan` (slot 6) rendering as **10** and **14**. `Color::to_crossterm` maps `Green => CtColor::Green` and `BrightGreen => CtColor::DarkGreen`, which crossterm emits as `5;10` and `5;2` — inverted — while `palette_index()` says `Green => 2` correctly and is used only by the markdown code-block path, never by the frame. The doc comment claiming "the two render paths now agree" is false for all six chromatic pairs, and the theme test that would have caught it asserts against `palette_index()`, the function the frame does not use. This is the one outright rendering defect in the sweep, not merely an untested claim._
...
- [-] **Shell Output Insert (`i`)** `P0` — `i` inserts the command's output into the composer · `crucible-cli`
  - **Gets you:** nothing. Pressing `i` closes the modal and inserts nothing. Same defect for `t`.
  - **Proof:** _none — `i` sets `pending_insert` then returns `ShellModalOutput::Close` in the same call; `InsertOutput` is only emitted on a subsequent `Tick`, but the app consumes `Close` immediately and drops the modal, so no `Tick` ever reaches it again. The failing test is **already written and `#[ignore]`d** (`shell_tests.rs::insert_key_should_emit_output_but_closes_first`); the companion that passes does so only by hand-pumping a `Tick` the real app never sends. Knowingly shipped._
...
- [-] **Viewport Caching** `P0` — cached messages, tool calls, shell executions, subagents · `crucible-cli`
  - **Gets you:** messages, tool calls and subagents are cached and render. **Shell executions never enter the cache** — run `!cargo build`, close the modal, and nothing about that command appears in the transcript. There is also no lazy line-wrapping.
  - **Proof:** _none for the last two claims — `ContainerList::add_shell_execution` has zero call sites, so `CachedShellExecution`, `ChatNode::ShellExecution` and its renderer are all dead; the code says why (`/// TODO: wire shell execution results into container state`, where `update_shell_modal` takes the history item and does `let _ = &history_item;`). No memoised wrap exists anywhere in `viewport_cache.rs`. The working three are snapshot-proven._
...
- [-] **Type Stub Coverage** `P1` — the stubs describe the real `cru.*` surface · `crucible-lua`
  - **Gets you:** autocomplete that offers six namespaces which do not exist and hides fourteen that do.
  - **Proof:** _none — the generator does not walk the registered namespaces; it walks a hardcoded `UNIVERSAL_MODULES` list inside a throwaway executor that first *fabricates* `cru.ask`, `cru.graph`, `cru.mcp`, `cru.hooks`, `cru.notify` and `cru.session` out of bare globals. None of those six exist on the VM that runs plugins. Meanwhile `check config emitter errors health json log schedule service shell statusline storage ws` (plus `fmt inspect retry spawn tbl_deep_extend tbl_get`) get no stubs at all, and `cru.defaults` / `cru.modes` appear nowhere. The file's own comment states the rule this violates: "A stub for a nonexistent function is worse than a stale doc: it looks authoritative." The two stub directories also disagree — the daemon writes `~/.config/crucible/luals/` while `cru plugin stubs` defaults to `~/.config/crucible/stubs/`._
...
- [-] **`.luarc.json` Generation** `P2` — scaffolded `.luarc.json` points at the type stubs for zero-config IDE setup · `crucible-cli`
  - **Gets you:** a `.luarc.json`, but one that does not reference the stubs — so IDE setup is not zero-config. It also lists `cru` in `diagnostics.globals` but not `crucible`, so the long-form alias is flagged as undefined.
  - **Proof:** _none — hands-on counter-evidence: the emitted file is `"workspace": { "library": ["${3rd}/luv/library"], "checkThirdParty": false }` with no reference to the stub directory, and the template is a static `include_str!` with no substitution. `cru plugin stubs` even prints instructions telling the user to paste the path in by hand, which is the command's own UX conceding the feature does not exist. `cru plugin init`, named in the old entry, is not a subcommand at all._
...
- [-] **File Watching** `P0` — file change detection with auto-reprocessing · `crucible-daemon` (watch)
  - **Gets you:** the watcher starts and the OS backend fires, and nothing downstream consumes the events. **No** file change auto-reprocesses — created or modified.
  - **Proof:** _none — the handler registry is empty in production: `watch/handlers/mod.rs:85-96` builds a `HandlerRegistry::new()` and returns it with the only registration commented out (`// TODO: Re-enable indexing handler when feature flag is ready`). `WatchManager` copies that empty registry, and the `DaemonEventBridge` emitter it is handed has no call site anywhere in `manager.rs` except its getter. So `IndexingHandler::emit_session_event` never runs → `FileChanged` is never emitted → the reprocess task at `server/mod.rs:517-560`, which is fully written and correct, never receives an event. The four `watch_*_emission_tests.rs` files are why this went unnoticed: they construct `IndexingHandler::with_emitter(MockEventEmitter)` by hand and assert the mock captured the event — they prove the handler works and say nothing about whether it is ever registered. A ~30-line daemon test (open a kiln, write a `.md`, poll `list_notes`) would fail today._
...
- [-] **Agent Config** `P0` — default agent, temperature, max_tokens, thinking budget · [[Help/Config/agents]] · `crucible-core` (config)
  - **Gets you:** thinking budget reaches the model (see **Extended Thinking**). Temperature, max_tokens and "default agent" do not.
  - **Proof:** _none for three of four — `GenaiAgentHandle` has no temperature or max_tokens field; its constructor takes `(client, model, system_prompt, tools, thinking_budget, workspace_root)` and the `ChatOptions` built for every request sets only capture flags plus reasoning effort. `session.set_temperature` writes the field, emits `temperature_changed`, and stops; `AgentHandle::set_temperature`'s default impl returns `NotSupported` and `GenaiAgentHandle` does not override it. For ACP agents it is a pure cache that nothing forwards to the process. `acp.default_agent` is read in exactly two places, both of which only print it. The existing tests are why this survived: `test_temperature_round_trip`, `test_max_tokens_round_trip` and `all_config_knobs_round_trip_over_the_wire` are set-then-get through RPC by name, and they pass while the knob is inert._
