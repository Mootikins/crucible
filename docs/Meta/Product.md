...
- [-] **Reflection Pass** `P2` — on `on_session_end`, a forked cheap-model subagent reviews the finished transcript and proposes kiln notes · [[Help/Concepts/Reflection Pass]] · `crucible-daemon`, `crucible-lua`, `runtime/plugins/reflection`
  ...
  - **Proof:** _none — the shipped Lua suite covers `count_user_turns`, `build_transcript`, `parse_proposals`, `render_proposal` and the recursion guard, but `M.stage_proposals` and `M.run`'s happy path have no test at all; the `enabled`/`min_turns`/`model` knobs are covered only as config round-trips. The packaging half of this is now fixed — the plugin does reach installed users (see **Bundled Runtime Plugins in Releases**) — so the missing coverage above is all that stands between this and `[x]`._
...
- [x] **Bundled Help Skills** `P1` — help skills shipped at `runtime/crucible-help/skills` · `crucible-daemon` (skills)
  ...
  - **Proof:** `crates/crucible-daemon/src/skills/discovery.rs`::skills_extracted_from_the_binary_are_discovered runs the whole path an installed user takes — bytes compiled into `cru`, extracted to a directory, found by the real discovery code; `::an_installed_binary_finds_the_bundled_runtime_skills` and `::a_dev_tree_binary_still_finds_the_bundled_runtime_skills` hold the two on-disk layouts. Both halves are now done: the resolver drifted (it omitted the installed layout the other three tried first, fixed by sharing `crucible_core::runtime_roots`) and the packaging never delivered a tree for it to find.
...
- [x] **`cru setup`** `P0` — bootstrap the runtime (plugins, themes, default `init.lua`) into `~/.config/crucible/runtime` and print the `runtimepath` line to add · `crucible-cli`
  - **Gets you:** a runtime tree you can edit, whether or not one shipped on disk — with no source to copy from it writes the copy compiled into the binary, and `--force` restores the shipped files over your edits.
  - **Proof:** `crates/crucible-cli/src/commands/setup.rs`::setup_writes_the_compiled_in_tree_when_nothing_is_installed, `::setup_restores_a_hand_edited_tree_rather_than_trusting_its_stamp`, `::an_installed_tree_is_copied_in_preference_to_the_compiled_in_one`. It used to bail with "Could not find Crucible runtime files" for exactly the users it exists to serve, because it resolved its *source* the same exe-relative way the daemon does and no release put anything there.
