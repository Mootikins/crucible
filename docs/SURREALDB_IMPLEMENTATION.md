...
1. **Schema Design**
   - `./docs/SURREALDB_SCHEMA.md` - Architecture document with comprehensive design rationale
   - `./crucible/crates/crucible-surrealdb/src/schema.surql` - SurrealQL schema definitions

2. **Rust Types**
   - `./crucible/crates/crucible-surrealdb/src/schema_types.rs` - Type-safe Rust definitions matching schema

3. **Query Examples**
   - `./crucible/crates/crucible-surrealdb/examples/queries.surql` - Comprehensive query patterns
...
Add SurrealDB dependency to `./crucible/crates/crucible-surrealdb/Cargo.toml`:
